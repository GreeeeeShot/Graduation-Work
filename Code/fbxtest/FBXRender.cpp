...
namespace FBX_LOADER
{
	...
	HRESULT CFBXRenderDX11::CreateNodes(ID3D11Device*	pd3dDevice)
	{
		...
		for (size_t i = 0; i<nodeCoount; i++)
		{
			...
			VertexConstruction(pd3dDevice, fbxNode, meshNode); //버퍼를meshnode에 옮긴다.
			...
			MaterialConstruction(pd3dDevice, fbxNode, meshNode); //조명값에 대한것같음
	...
	HRESULT CFBXRenderDX11::VertexConstruction(ID3D11Device*	pd3dDevice, FBX_MESH_NODE &fbxNode, MESH_NODE& meshNode)
	{
		...
		for (size_t i = 0; i<meshNode.vertexCount; i++) //버텍스갯수
		{
			FbxVector4 v = fbxNode.m_positionArray[i];
			pV[i].vPos = DirectX::XMFLOAT3((float)v.mData[0], //각버텍스의 xyz값을 저장
				(float)v.mData[1],
				(float)v.mData[2]);

			v = fbxNode.m_normalArray[i];

			pV[i].vNor = DirectX::XMFLOAT3((float)v.mData[0],//각버텍스의 노말값을 저장
				(float)v.mData[1],
				(float)v.mData[2]);

			if ((float)fbxNode.m_texcoordArray.size() > 0) //tex의 관한 정보를 넣는듯
			{
				...
				pV[i].vTexcoord = DirectX::XMFLOAT2(0, 0); //tex가 없으면 0, 0으로
		}

		CreateVertexBuffer(pd3dDevice, &meshNode.m_pVB, pV, sizeof(VERTEX_DATA), meshNode.vertexCount); // 버텍스 버퍼생성
	...
	HRESULT CFBXRenderDX11::MaterialConstruction(ID3D11Device*	pd3dDevice, FBX_MESH_NODE &fbxNode, MESH_NODE& meshNode)
	{
		...
		if (fbxMaterial.diffuse.textureSetArray.size()>0)
		{
			...
			if (it->second.size())
			{
				...
				//CreateDDSTextureFromFile(pd3dDevice, wstr, NULL, &meshNode.materialData.pSRV, 0);	// DXTex궔귞
			}
		}
