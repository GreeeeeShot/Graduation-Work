...
void CTexturedLightingGridMesh::Animate(ID3D11DeviceContext *pd3dDeviceContext, float fTimeElapsed)
{
	...
	//{
		...
		for (int i = 0; i < m_iLength; i++)
		{
			for (int j = 0; j < m_iWidth; j++)
			{
				fDerI = j % 2 == 0 && i % 2 == 0 ? -cos(fDynamicRadian2 + (i + j)) * 0.15f : cos(fDynamicRadian + (i + j)) * 0.15f;
				fDerJ = j % 2 == 0 && i % 2 == 0 ? -cos(fDynamicRadian2 + (i + j)) * 0.15f : cos(fDynamicRadian + (i + j)) * 0.15f;
				fNorm = D3DXVECTOR3(-2.5f*fDerI, 1, -2.5f*fDerJ);

				D3DXVec3Normalize(&fNorm, &fNorm);
				
				pVertices[iVNum++] = CNormalMappingVertex(
					D3DXVECTOR3(m_fStartX + (float)j * m_fIntervalX, 
						j % 2 == 0 && i % 2 == 0 ? -sin(fDynamicRadian2+(i+j)) * 0.15f : sin(fDynamicRadian + (i+j)) * 0.15f + 0.5f, 
						m_fStartZ - (float)i * m_fIntervalZ),
					fNorm,
					D3DXVECTOR3(j / 4.0f -  0.015f*cos(fFlowCoefficient), i / 4.0f - 0.015f * sin(fDynamicRadian), fUpdatingTime),
					D3DXVECTOR3(1.0f, 0.0f, 0.0f));
			}
		}
