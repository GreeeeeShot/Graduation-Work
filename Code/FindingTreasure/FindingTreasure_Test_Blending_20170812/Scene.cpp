...
void CScene::AnimateObjects(ID3D11Device *pd3dDevice, ID3D11DeviceContext*pd3dDeviceContext, float fTimeElapsed)
{
	...
	for (int i = 0; i < m_pPlayersMgrInform->m_iPlayersNum; i++)
	{
		if (m_pPlayersMgrInform->m_ppPlayers[i])
		{
			if (m_pPlayersMgrInform->m_ppPlayers[i]->m_bIsActive)
			{
				D3DXVECTOR3 d3dxvVel = m_pPlayersMgrInform->m_ppPlayers[i]->m_d3dxvVelocity;

				if (D3DXVec2Length(&D3DXVECTOR2(d3dxvVel.x, d3dxvVel.z)) < 0.3f)
				{
					if (!m_pPlayersMgrInform->m_ppPlayers[i]->m_pLiftedPlayer) m_pPlayersMgrInform->m_ppPlayers[i]->IncreaseStamina(fTimeElapsed);		// 움직이지 않고 있다면,
					else m_pPlayersMgrInform->m_ppPlayers[i]->IncreaseStamina(fTimeElapsed / 4.0f);
				}
				else
				{
					if (m_pPlayersMgrInform->m_ppPlayers[i]->m_pLiftedPlayer) m_pPlayersMgrInform->m_ppPlayers[i]->DecreaseStamina(fTimeElapsed);
					else m_pPlayersMgrInform->m_ppPlayers[i]->IncreaseStamina(fTimeElapsed);
				}
			}
		}
	}
	...
	//if (m_pTreasureChest->GetPosition().y < 0.5f)
	//{
...
CJungleScene::CJungleScene() : CScene("new mapfile test/jungle0.txt")
{
...
CSnowScene::CSnowScene() : CScene("new mapfile test/snow0.txt")
{
