...
D3DXVECTOR3* FBXExporter::CopyMeshVertex(int* vertices)
{
	D3DXVECTOR3* vec = new D3DXVECTOR3[mTriangles.size()*3];

	for (int i = 0; i < mTriangles.size(); ++i)
	{
		vec[i * 3].x = mVertices[mTriangles[i].mIndices[0]].mPosition.x * 0.5;
		vec[i * 3].y = mVertices[mTriangles[i].mIndices[0]].mPosition.y* 0.5;
		vec[i * 3].z = mVertices[mTriangles[i].mIndices[0]].mPosition.z* 0.5;
		vec[i * 3+1].x = mVertices[mTriangles[i].mIndices[1]].mPosition.x* 0.5;
		vec[i * 3+1].y = mVertices[mTriangles[i].mIndices[1]].mPosition.y* 0.5;
		vec[i * 3+1].z = mVertices[mTriangles[i].mIndices[1]].mPosition.z* 0.5;
		vec[i * 3+2].x = mVertices[mTriangles[i].mIndices[2]].mPosition.x* 0.5;
		vec[i * 3+2].y = mVertices[mTriangles[i].mIndices[2]].mPosition.y* 0.5;
		vec[i * 3+2].z = mVertices[mTriangles[i].mIndices[2]].mPosition.z* 0.5;
	}
	*vertices = mTriangles.size() * 3;

	return vec;
}

D3DXVECTOR3* FBXExporter::CopyNormalVertex()
{
	D3DXVECTOR3* nor = new D3DXVECTOR3[mTriangles.size() * 3];

	for (int i = 0; i < mTriangles.size(); ++i)
	{
		nor[i * 3].x = mVertices[mTriangles[i].mIndices[0]].mNormal.x;
		nor[i * 3].y = mVertices[mTriangles[i].mIndices[0]].mNormal.y;
		nor[i * 3].z = mVertices[mTriangles[i].mIndices[0]].mNormal.z;
		nor[i * 3 + 1].x = mVertices[mTriangles[i].mIndices[1]].mNormal.x;
		nor[i * 3 + 1].y = mVertices[mTriangles[i].mIndices[1]].mNormal.y;
		nor[i * 3 + 1].z = mVertices[mTriangles[i].mIndices[1]].mNormal.z;
		nor[i * 3 + 2].x = mVertices[mTriangles[i].mIndices[2]].mNormal.x;
		nor[i * 3 + 2].y = mVertices[mTriangles[i].mIndices[2]].mNormal.y;
		nor[i * 3 + 2].z = mVertices[mTriangles[i].mIndices[2]].mNormal.z;
	}
	return nor;
}

D3DXVECTOR2* FBXExporter::CopyUVVertex()
{
	D3DXVECTOR2* uv = new D3DXVECTOR2[mTriangles.size() * 3];

	for (int i = 0; i < mTriangles.size(); ++i)
	{
		uv[i * 3].x = mVertices[mTriangles[i].mIndices[0]].mUV.x;
		uv[i * 3].y = 1.0 - mVertices[mTriangles[i].mIndices[0]].mUV.y;
		uv[i * 3 + 1].x = mVertices[mTriangles[i].mIndices[1]].mUV.x;
		uv[i * 3 + 1].y = 1.0 - mVertices[mTriangles[i].mIndices[1]].mUV.y;
		uv[i * 3 + 2].x = mVertices[mTriangles[i].mIndices[2]].mUV.x;
		uv[i * 3 + 2].y = 1.0 - mVertices[mTriangles[i].mIndices[2]].mUV.y;
	}
	return uv;
}
...
void FBXExporter::AnimationToVertex(D3DXVECTOR3** vec,int Frame_t)
{
	*vec = new D3DXVECTOR3[mTriangles.size()*3];
	int sceneTime = (Frame_t*2)%mAnimationLength;
	for (int i = 0; i < mTriangleCount; ++i)
	{
		for (int k = 0; k < 3; ++k)
		{
			FbxAMatrix TranslationOfVectex;
			MatrixScale(TranslationOfVectex, 0.0);
			for (int j = 0; j < mVertices[mTriangles[i].mIndices[k]].mVertexBlendingInfos.size(); ++j)
			{
				if (!(mVertices[mTriangles[i].mIndices[k]].mVertexBlendingInfos[j].mBlendingIndex))
					break;
				FbxAMatrix SkeletonInverse = mSkeleton.mJoints[mVertices[mTriangles[i].mIndices[k]].mVertexBlendingInfos[j].mBlendingIndex].mGlobalBindposeInverse;
				Keyframe** keyframe=NULL;
				keyframe = &(mSkeleton.mJoints[mVertices[mTriangles[i].mIndices[k]].mVertexBlendingInfos[j].mBlendingIndex].mAnimation);
				for (int n = 0; n < sceneTime; ++n)
				{
					keyframe = &((**keyframe).mNext);
				}
				FbxAMatrix currentMatrix =  (**keyframe).mGlobalTransform * SkeletonInverse;
				MatrixScale(currentMatrix, mVertices[mTriangles[i].mIndices[k]].mVertexBlendingInfos[j].mBlendingWeight);
				MatrixAdd(TranslationOfVectex,currentMatrix);
			}
			D3DXVECTOR3 VertexPos(mVertices[mTriangles[i].mIndices[k]].mPosition.x, mVertices[mTriangles[i].mIndices[k]].mPosition.y, mVertices[mTriangles[i].mIndices[k]].mPosition.z);
			D3DXMATRIX MoveMatrix;
			MoveMatrix._11 = TranslationOfVectex.mData[0].mData[0];
			MoveMatrix._12 = TranslationOfVectex.mData[0].mData[1];
			MoveMatrix._13 = TranslationOfVectex.mData[0].mData[2];
			MoveMatrix._14 = TranslationOfVectex.mData[0].mData[3];
			MoveMatrix._21 = TranslationOfVectex.mData[1].mData[0];
			MoveMatrix._22 = TranslationOfVectex.mData[1].mData[1];
			MoveMatrix._23 = TranslationOfVectex.mData[1].mData[2];
			MoveMatrix._24 = TranslationOfVectex.mData[1].mData[3];
			MoveMatrix._31 = TranslationOfVectex.mData[2].mData[0];
			MoveMatrix._32 = TranslationOfVectex.mData[2].mData[1];
			MoveMatrix._33 = TranslationOfVectex.mData[2].mData[2];
			MoveMatrix._34 = TranslationOfVectex.mData[2].mData[3];
			MoveMatrix._41 = TranslationOfVectex.mData[3].mData[0];
			MoveMatrix._42 = TranslationOfVectex.mData[3].mData[1];
			MoveMatrix._43 = TranslationOfVectex.mData[3].mData[2];
