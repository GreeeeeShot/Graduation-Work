...
void CShadowRenderer::DestroyShadowRenderer(void)
{
	if(m_pShadowMapDSV) m_pShadowMapDSV->Release();		// pass0 -> 상대적 깊이를 기록하는데 쓰인다.
	if (m_pShadowMapSRV) m_pShadowMapSRV->Release();	// pass1 -> 실제 쉐도우매핑을 하는데 쓰인다.
	if (m_pd3dcbLightEmit) m_pd3dcbLightEmit->Release();
	if (m_pd3dcbShadowMatrix) m_pd3dcbShadowMatrix->Release();
	if (m_pSamplerPCF) m_pSamplerPCF->Release();
}
...
void CShadowRenderer::SetLightSource(CLightSource* pLightSource)
{
...
void CShadowRenderer::UpdateShaderPass0Variables(ID3D11DeviceContext *pd3dDeviceContext)
{
	...
	D3DXMatrixTranspose(&pcbViewProjection->m_d3dxmtxView, &m_pLightSource->m_d3dxmtxView);
	D3DXMatrixTranspose(&pcbViewProjection->m_d3dxmtxProjection, &m_pLightSource->m_d3dxmtxProjection);
...
void CShadowRenderer::UpdateShaderPass1Variables(ID3D11DeviceContext *pd3dDeviceContext)
{
	...
	D3DXMatrixMultiply(&m_d3dxmtxShadowMatrix, &m_pLightSource->m_d3dxmtxView, &m_pLightSource->m_d3dxmtxProjection);
