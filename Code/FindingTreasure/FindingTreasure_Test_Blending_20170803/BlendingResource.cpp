...
void CBlendingResource::CreateBlendingResource(ID3D11Device *pd3dDevice)
{
	...
	addDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;
...
void CBlendingResource::DestroyBlendingResource(void)
{
	if (pTransparentBlending) pTransparentBlending->Release();
	if (pAddBlending) pAddBlending->Release();
}