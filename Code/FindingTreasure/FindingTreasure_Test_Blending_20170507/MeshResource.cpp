...
CTexturedLightingCharacterMesh::CTexturedLightingCharacterMesh(ID3D11Device *pd3dDevice) : CLightingMesh(pd3dDevice)
{
	float fx = 0.5f, fy = 0.8f, fz = 0.5f;
	m_AABB.m_d3dxvMax = D3DXVECTOR3(fx, fy, fz);
	m_AABB.m_d3dxvMin = D3DXVECTOR3(-fx, -fy, -fz);

	m_AniWalk = new FBXExporter();
	m_AniWalk->Initialize();
	m_AniWalk->LoadScene("P_Walk.FBX");
	m_AniWalk->ImportFBX();

	m_AniIdle = new FBXExporter();
	m_AniIdle->Initialize();
	m_AniIdle->LoadScene("P_standing.FBX");
	m_AniIdle->ImportFBX();

	m_AniAtk = new FBXExporter();
	m_AniAtk->Initialize();
	m_AniAtk->LoadScene("P_standing.FBX");
	m_AniAtk->ImportFBX();

	m_AniMakeAHole = new FBXExporter();
	m_AniMakeAHole->Initialize();
	m_AniMakeAHole->LoadScene("P_standing.FBX");
	m_AniMakeAHole->ImportFBX();

	m_AniHit = new FBXExporter();
	m_AniHit->Initialize();
	m_AniHit->LoadScene("P_standing.FBX");
	m_AniHit->ImportFBX();
	...
	vec = m_AniIdle->CopyMeshVertex(&m_nVertices);
	nor = m_AniIdle->CopyNormalVertex();
	uv = m_AniIdle->CopyUVVertex();

	CTexturedLightingVertex* pVertices = new CTexturedLightingVertex[m_nVertices];

	for (int i = 0; i < m_nVertices; ++i)
	{
		pVertices[i] = CTexturedLightingVertex(D3DXVECTOR3(vec[i].x*0.01, vec[i].y*0.01, vec[i].z*0.01), nor[i], uv[i]);
	}
...
void CTexturedLightingCharacterMesh::Animate(ID3D11DeviceContext *pd3dDeviceContext, float t)
{
	//정점 버퍼를 매핑하여 정점 버퍼 포인터를 가져온다.
	D3D11_MAPPED_SUBRESOURCE d3dMappedResource;
	pd3dDeviceContext->Map(m_ppd3dVertexBuffers[0], 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedResource);
	...
	for (int i = 0; i < m_nVertices; ++i)
	{
		Vertices[i] = CTexturedLightingVertex(D3DXVECTOR3(vec[i].x*0.01, vec[i].y*0.01, vec[i].z*0.01), nor[i], uv[i]);
	}
...
void CMeshResource::CreateMeshResource(ID3D11Device *pd3dDevice)
{
	...
	pPirateMesh = new CTexturedLightingCharacterMesh(pd3dDevice);
