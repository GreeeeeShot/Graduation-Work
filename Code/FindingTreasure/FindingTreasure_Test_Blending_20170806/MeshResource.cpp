			...
			for (int j = 0; j < m_iWidth; j++)
			{
				...
				pVertices[iVNum++] = CNormalMappingVertex(
					D3DXVECTOR3(m_fStartX + (float)j * m_fIntervalX, 
						j % 2 == 0 && i % 2 == 0 ? -sin(fDynamicRadian2+(i+j)) * 0.25f : sin(fDynamicRadian + (i+j)) * 0.25f + 0.5f, 
...
void CMeshResource::CreateMeshResource(ID3D11Device *pd3dDevice)
{
	...
	pTexturedRectMesh = new CTexturedRectMesh(pd3dDevice, 1, 1, d3dxvUV);
	pUI_VoxelPocketMesh = new CTexturedRectMesh(pd3dDevice, 5, 1, d3dxvUV);
	//pEffect_ParticleMesh = new CTexturedRectMesh(pd3dDevice, 24, 0.5f, d3dxvUV);
	pUI_VoxelActivatedMesh = new CTexturedRectMesh(pd3dDevice, 1, 1, d3dxvUV);

	d3dxvUV[0] = D3DXVECTOR2(0.0f, 6.0f/7.0f);
	d3dxvUV[1] = D3DXVECTOR2(0.0f, 0.0f);
	d3dxvUV[2] = D3DXVECTOR2(1.0f, 6.0f / 7.0f);
	d3dxvUV[3] = D3DXVECTOR2(1.0f, 0.0f);
	pUI_RespawningTextMesh = new CTexturedRectMesh(pd3dDevice, 25, 3, d3dxvUV);

	d3dxvUV[0] = D3DXVECTOR2(0.0f, 1.0f);
	d3dxvUV[1] = D3DXVECTOR2(0.0f, 6.0f / 7.0f);
	d3dxvUV[2] = D3DXVECTOR2(1.0f, 1.0f);
	d3dxvUV[3] = D3DXVECTOR2(1.0f, 6.0f / 7.0f);
	pUI_RespawnigGaugeMesh = new CTexturedRectMesh(pd3dDevice, 24, 0.5f, d3dxvUV);

	d3dxvUV[0] = D3DXVECTOR2(0.0f, 2.0f/3.0f);
	d3dxvUV[1] = D3DXVECTOR2(0.0f, 0.0f);
	d3dxvUV[2] = D3DXVECTOR2(1.0f, 2.0f / 3.0f);
	d3dxvUV[3] = D3DXVECTOR2(1.0f, 0.0f);
	pUI_StaminaMesh = new CTexturedRectMesh(pd3dDevice, 11, 1, d3dxvUV);

	d3dxvUV[0] = D3DXVECTOR2(0.0f, 1.0f);
	d3dxvUV[1] = D3DXVECTOR2(0.0f, 2.0f / 3.0f);
	d3dxvUV[2] = D3DXVECTOR2(1.0f, 1.0f);
	d3dxvUV[3] = D3DXVECTOR2(1.0f, 2.0f / 3.0f);
	pUI_StaminaGaugeMesh = new CTexturedRectMesh(pd3dDevice, 10, 0.5f, d3dxvUV);
}

void CMeshResource::DestroyMeshResource(void)
{
	if(pSkyMesh) delete pSkyMesh;
	if (pAABBMesh) delete pAABBMesh;
	if (pShipMesh) delete pShipMesh;
	if (pTreasureChestMesh) delete pTreasureChestMesh;
	if (pTexturedRectMesh) delete pTexturedRectMesh;
	if (pWaveMesh) delete pWaveMesh;
	if (pStandardVoxelMesh) delete pStandardVoxelMesh;
	if (pUphill000Mesh) delete pUphill000Mesh;
	if (pUphill090Mesh) delete pUphill090Mesh;
	if (pUphill180Mesh) delete pUphill180Mesh;
	if (pUphill270Mesh) delete  pUphill270Mesh;
	if (pUI_VoxelPocketMesh) delete pUI_VoxelPocketMesh;
	if (pUI_VoxelActivatedMesh) delete pUI_VoxelActivatedMesh;
	if (pUI_RespawningTextMesh) delete pUI_RespawningTextMesh;
	if (pUI_RespawnigGaugeMesh) delete pUI_RespawnigGaugeMesh;
	if (pUI_StaminaMesh) delete pUI_StaminaMesh;
	if (pUI_StaminaGaugeMesh) delete pUI_StaminaGaugeMesh;
	if (pEffect_ParticleMesh) delete pEffect_ParticleMesh;
}