...
void CMaterialResource::CreateMaterialResource(ID3D11Device *pd3dDevice)
{
	...
	pStandardMaterial->m_Material.m_d3dxcDiffuse = D3DXCOLOR(0.6f, 0.6f, 0.6f, 1.0f);
	pStandardMaterial->m_Material.m_d3dxcAmbient = D3DXCOLOR(0.45f, 0.45f, 0.45f, 1.0f);
	pStandardMaterial->m_Material.m_d3dxcSpecular = D3DXCOLOR(0.2f, 0.2f, 0.4f, 1.0f);
...
void CMaterialResource::DestroyMaterialResource(void)
{
	if(pStandardMaterial) delete pStandardMaterial;
	if (pWaveMaterial) delete pWaveMaterial;
	if (pHalfTransMaterial) delete pHalfTransMaterial;
	if (pHalfMoreTransMaterial) delete pHalfMoreTransMaterial;
	if (pHalfLessTransMaterial) delete pHalfLessTransMaterial;
}