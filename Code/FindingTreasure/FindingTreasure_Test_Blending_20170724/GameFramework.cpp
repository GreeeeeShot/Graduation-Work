...
void CGameFramework::BuildObjects()
{
	...
	m_pShadowRenderer->m_cLightSource.SetMesh(new CSphereMesh(m_pd3dDevice, 5.0f, 20, 20, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f)));
	...
	m_pShadowRenderer->m_cLightSource.CreateViewMatrix(
		D3DXVECTOR3(-0.0f, 40.0f, -60.0f), D3DXVECTOR3(-0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 1.0f, 0.0f));
...
void CGameFramework::FrameAdvance()
{
	...
	if (m_pShadowRenderer)    // 쉐도우 렌더러가 있다면 그림자 매핑을 수행한다.
	{
		...
		if (m_pLightShaftRenderer)
		{
			...
			float fLightMapBackColor[4] = { 0.6f, 0.3f, 0.1f, 1.0f };
			m_pd3dDeviceContext->ClearRenderTargetView(m_pLightShaftRenderer->m_pLightMapRTV, fLightMapBackColor); // 라이트맵에 배경화면을 그린다.
			m_pLightShaftRenderer->m_pLightSource->Render(m_pd3dDeviceContext); // 광원을 그린다.
		...
		{
			m_pd3dDeviceContext->OMSetRenderTargets(2, &m_pd3dRenderTargetView, m_pd3dDepthStencilView);
		}
		//m_pd3dDeviceContext->OMSetRenderTargets(1, &m_pd3dRenderTargetView, m_pd3dDepthStencilView);
