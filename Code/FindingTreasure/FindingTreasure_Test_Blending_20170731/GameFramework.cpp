...
void CGameFramework::BuildObjects()
{
	...
	pLightSource->SetMesh(new CSphereMesh(m_pd3dDevice, 5.0f, 20, 20, D3DXCOLOR(2.5f, 2.5f, 2.5f, 1.0f)));
	pLightSource->SetShader(CShaderResource::pDiffusedShader);
	pLightSource->CreateViewMatrix(
		D3DXVECTOR3(-2.0f, 380.0f, -600.0f), D3DXVECTOR3(-0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 1.0f, 0.0f));
	pLightSource->CreateProjectionMatrix(4.2f, m_nWndClientWidth, m_nWndClientHeight, 650.0f, 800.0f);

	m_pShadowRenderer->SetLightSource(pLightSource);
	...
	m_pScene = new CScene("new mapfile test/snow0.txt");
	//플레이어 객체를 생성한다.
	...
	for (int i = 0; i < MAX_CONNECTED_PLAYERS_NUM; i++)			// 최대 8명이 접속한다고 가정함.
	{
		sWaitingRoomInformDesc.m_PlayerInformDesc[i].m_bIsSlotActive = true;
		sWaitingRoomInformDesc.m_PlayerInformDesc[i].m_PlayerType = PLAYER_TYPE_PIRATE;
		sWaitingRoomInformDesc.m_PlayerInformDesc[i].m_BelongType = i < 4 ? BELONG_TYPE_BLUE : BELONG_TYPE_RED;
	}
