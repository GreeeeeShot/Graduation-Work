...
void CScene::CreateShaderVariables(ID3D11Device *pd3dDevice)
{
	...
	m_pLights->m_d3dxcGlobalAmbient = D3DXCOLOR(0.05f, 0.1f, 0.1f, 1.0f);
	...
	m_pLights->m_pLights[1].m_fRange = 400.0f;
	m_pLights->m_pLights[1].m_d3dxcAmbient = D3DXCOLOR(0.7f, 0.5f, 0.5f, 1.0f);
	m_pLights->m_pLights[1].m_d3dxcDiffuse = D3DXCOLOR(0.9f, 0.6f, 0.3f, 1.0f);
	m_pLights->m_pLights[1].m_d3dxcSpecular = D3DXCOLOR(1.2f, 1.0f, 1.0f, 10.0f);
	m_pLights->m_pLights[1].m_d3dxvPosition = D3DXVECTOR3(-0.0f, 45.0f, -250.0f);
	m_pLights->m_pLights[1].m_d3dxvDirection = D3DXVECTOR3(0.0f, -1.0f, 6.6f);
	m_pLights->m_pLights[1].m_d3dxvAttenuation = D3DXVECTOR3(1.0f, 0.0f, 0.0000f);
	...
	m_pLights->m_pLights[2].m_d3dxcAmbient = D3DXCOLOR(0.1f, 0.1f, 0.13f, 1.0f);
	...
	m_pLights->m_pLights[2].m_d3dxvDirection = D3DXVECTOR3(-0.0f, -5.0f, 5.0f);
	...
	m_pLights->m_pLights[4].m_d3dxcAmbient = D3DXCOLOR(0.1f, 0.1f, 0.13f, 1.0f);
	...
	m_pLights->m_pLights[4].m_d3dxvDirection = D3DXVECTOR3(0.0f, -5.0f, -5.0f);
	...
	m_pLights->m_pLights[6].m_d3dxcAmbient = D3DXCOLOR(0.1f, 0.1f, 0.13f, 1.0f);
	...
	m_pLights->m_pLights[6].m_d3dxvDirection = D3DXVECTOR3(5.0f, -5.0f, -0.0f);
...
*/
	...
	m_pLights->m_pLights[7].m_d3dxcAmbient = D3DXCOLOR(0.1f, 0.1f, 0.13f, 1.0f);
	...
	m_pFog->m_fFogEnable = 1.0f;
	m_pFog->m_fFogStart = 1.0f;
	m_pFog->m_fFogRange = 160.0f;
...
void CScene::MoveObjectUnderPhysicalEnvironment(CPlayer *pPlayer, float fTimeElapsed)
{
	...
	if (bIsJumped)
	{
		d3dxvPostV.y = pPlayer->m_d3dxvVelocity.y = 4.97f;//4.95f;// +d3dxvPostV.y;
	}
...
void CScene::AnimateObjects(ID3D11DeviceContext*pd3dDeviceContext, float fTimeElapsed)
{
	...
	for (int i = 0; i < m_pPlayersMgrInform->m_iPlayersNum; i++)
	{
		if (m_pPlayersMgrInform->m_ppPlayers[i]->m_bIsActive)
		{
			...
			{
				CPhysicalCollision::ImpurseBasedCollisionResolution(m_pPlayersMgrInform->m_ppPlayers[i], m_pTreasureChest);
