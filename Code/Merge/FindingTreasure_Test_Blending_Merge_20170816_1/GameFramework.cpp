...
void CGameFramework::OnDestroy()
{
	...
	if (m_pd3dRenderTargetView)
	{
		m_pd3dRenderTargetView->Release();		// 완전 소멸 가정
		m_pd3dRenderTargetView = NULL;
	}
	if (m_pd3dDepthStencilBuffer)
	{
		m_pd3dDepthStencilBuffer->Release();	// 완전 소멸 가정
		m_pd3dDepthStencilBuffer = NULL;
	}
	if (m_pd3dDepthStencilView)
	{
		m_pd3dDepthStencilView->Release();		// 완전 소멸 가정
		m_pd3dDepthStencilView = NULL;
	}
	if (m_pDXGISwapChain)
	{
		m_pDXGISwapChain->Release();					// 완전 소멸 가정
		m_pDXGISwapChain = NULL;
	}
	if (m_pd3dDeviceContext)
	{
		m_pd3dDeviceContext->Release();			// 완전 소멸 가정
		m_pd3dDeviceContext = NULL;
	}
	if (m_pd3dDevice)
	{
		m_pd3dDevice->Release();							// 완전 소멸 가정
		m_pd3dDevice = NULL;
	}
