...
public:
	...
	virtual float GetHeight(float fWorldX, float fWorldZ) { return -1.0f; }	// -1.0f는 일종의 에러 표시다.
	virtual void Render(ID3D11DeviceContext *pd3dDeviceContext);
	virtual void InstRender(ID3D11DeviceContext *pd3dDeviceContext, ID3D11Buffer* pd3dbInstBuf, UINT iInstSize, UINT iInstNum, UINT iInstStartIdx = 0);
};
...
public:
	...
	virtual void SetRasterizerState(ID3D11Device *pd3dDevice);
	float GetHeight(float fWorldX, float fWorldZ);
	//virtual void Render(ID3D11DeviceContext *pd3dDeviceContext);
	//격자 메쉬의 높이를 변화시킨다.
...
public:
	...
	static CMesh *pUI_StaminaMesh;
	static CMesh *pUI_StaminaGaugeMesh;
	static CMesh *pUI_KeyboardMesh;

	static CMesh *pEffect_ParticleMesh;

	static void CreateMeshResource(ID3D11Device *pd3dDevice);
	static void DestroyMeshResource(void);
};
