...
void CPlayer::ThrowPlayer(void)
{
	...
	//
	D3DXVECTOR3 d3dxvThrowDir;
	D3DXMATRIX d3dxmtxRot;
	D3DXMatrixRotationAxis(&d3dxmtxRot, &GetRight(), D3DXToRadian(-45.0f));
	D3DXVec3TransformNormal(&d3dxvThrowDir, &GetLook(), &d3dxmtxRot);

	float fFos = m_fCurStamina / PLAYER_MAX_STAMINA;
...
{
	if (m_d3dxvMoveDir.x != 0.0f || m_d3dxvMoveDir.y != 0.0f || m_d3dxvMoveDir.z != 0.0f) return true;
	else return false;
}
