...
CVoxelTerrain::CVoxelTerrain()
{
	...
	for (int i = 0; i < 4; i++)
	{
		m_pd3dSlopeVoxelInstanceBuffer[i] = NULL;
	}
}
...
CVoxelTerrain::~CVoxelTerrain()
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (m_pd3dSlopeVoxelInstanceBuffer[i]) m_pd3dSlopeVoxelInstanceBuffer[i]->Release();
	}
...
void CVoxelTerrain::CreateVoxelTerrain(ID3D11Device *pd3dDevice, CBlueprint* pBlueprint)
{
	...
	for (int i = 0; i < 4; i++)
	{
		m_pd3dSlopeVoxelInstanceBuffer[i] = CreateInstanceBuffer(pd3dDevice, m_iSlopeVoxelNum, sizeof(CTexturedLightingInstance), NULL);	// 수를 알 필요는 없을 듯
	}
	m_pd3dRectVoxelInstanceBuffer = CreateInstanceBuffer(pd3dDevice, m_iRectVoxelNum, sizeof(CTexturedLightingInstance), NULL);				// 수를 알 필요는 없을 듯
}
...
void CVoxelTerrain::Render(ID3D11DeviceContext *pd3dDeviceContext, CCamera *pCamera, bool bIsInstancing)
{
	...
	else // 인스턴싱
	{
		...
		int iVisibleSlopeVoxelNum[4] = {0, 0, 0, 0};
		int iSlopeType = 0;

		D3D11_MAPPED_SUBRESOURCE d3dMappedRectVoxelResource;			// 직육면체 복셀
		D3D11_MAPPED_SUBRESOURCE d3dMappedSlopeVoxelResource[4];			// 기울어진 복셀

		pd3dDeviceContext->Map(m_pd3dRectVoxelInstanceBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedRectVoxelResource);
		for (int i = 0; i < 4; i++)
		{
			pd3dDeviceContext->Map(m_pd3dSlopeVoxelInstanceBuffer[i], 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSlopeVoxelResource[i]);
		}
		
		CTexturedLightingInstance *pRectVoxelInstances = (CTexturedLightingInstance *)d3dMappedRectVoxelResource.pData;
		CTexturedLightingInstance *pSlopeVoxelInstances[4];
		for (int i = 0; i < 4; i++)
		{
			pSlopeVoxelInstances[i] = (CTexturedLightingInstance *)d3dMappedSlopeVoxelResource[i].pData;
		}
		
		m_pShader->UpdateShaderVariable(pd3dDeviceContext, CTextureResource::pd3dTextureArrayVoxel);		// 텍스쳐 배열이 세팅된다.
						...
						if (pCamera->m_bIsInnerCulling)
						{
							if (!IsInnerVoxel(m_vecVoxelObjectsForRendering[i][j], pCamera))
							{
								if (m_vecVoxelObjectsForRendering[i][j]->m_bIsVisible)
								{
									...
									{
										switch (m_vecVoxelObjectsForRendering[i][j]->m_eVoxelType)
										{
										case VOXEL_TYPE_ZUNGLE_UPHILL_0:
										case VOXEL_TYPE_SNOW_UPHILL_0:
											iSlopeType = 0;
										...
										D3DXMatrixTranspose(&pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_d3dxmtxWorld, &m_vecVoxelObjectsForRendering[i][j]->m_d3dxmtxWorld);
										pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_fTexType = GetTexture2DArrayIndexForType(m_vecVoxelObjectsForRendering[i][j]->m_eVoxelType);
										pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_d3dxvMtrlDiffuse = mtrl.m_d3dxcDiffuse;
										pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_d3dxvMtrlAmbient = mtrl.m_d3dxcAmbient;
										pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_d3dxvMtrlSpecular = mtrl.m_d3dxcSpecular;
										pSlopeVoxelInstances[iSlopeType][iVisibleSlopeVoxelNum[iSlopeType]].m_d3dxvMtrlEmissive = mtrl.m_d3dxcEmissive;

										iVisibleSlopeVoxelNum[iSlopeType]++;
									}
									else
									{
										D3DXMatrixTranspose(&pRectVoxelInstances[iVisibleRectVoxelNum].m_d3dxmtxWorld, &m_vecVoxelObjectsForRendering[i][j]->m_d3dxmtxWorld);
		...
		for (int i = 0; i < 4; i++)
		{
			pd3dDeviceContext->Unmap(m_pd3dSlopeVoxelInstanceBuffer[i], 0);
		}
		...
		//
		CMeshResource::pStandardVoxelMesh->InstRender(pd3dDeviceContext, m_pd3dRectVoxelInstanceBuffer, sizeof(CTexturedLightingInstance), iVisibleRectVoxelNum);
		CMeshResource::pUphill000Mesh->InstRender(pd3dDeviceContext, m_pd3dSlopeVoxelInstanceBuffer[0], sizeof(CTexturedLightingInstance), iVisibleSlopeVoxelNum[0]);
		CMeshResource::pUphill090Mesh->InstRender(pd3dDeviceContext, m_pd3dSlopeVoxelInstanceBuffer[1], sizeof(CTexturedLightingInstance), iVisibleSlopeVoxelNum[1]);
		CMeshResource::pUphill180Mesh->InstRender(pd3dDeviceContext, m_pd3dSlopeVoxelInstanceBuffer[2], sizeof(CTexturedLightingInstance), iVisibleSlopeVoxelNum[2]);
		CMeshResource::pUphill270Mesh->InstRender(pd3dDeviceContext, m_pd3dSlopeVoxelInstanceBuffer[3], sizeof(CTexturedLightingInstance), iVisibleSlopeVoxelNum[3]);
	}
}