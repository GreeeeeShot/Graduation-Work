...
CCharacterMesh::CCharacterMesh(ID3D11Device *pd3dDevice) : CMesh(pd3dDevice)
{
	...
	strcpy_s(m_filename, "walk_snow.FBX");
...
int CCharacterMesh::get_PositionVector(FBX_LOADER::CFBXLoader* pLoader)
{
	...
	for (int i = 0; i < nNode; ++i)
	{
		...
		{
			FbxVector4 vec;
			vec.mData[0] = meshNode[i].m_positionArray[j].mData[0]*0.1;
			vec.mData[1] = meshNode[i].m_positionArray[j].mData[1] * 0.1;
			vec.mData[2] = meshNode[i].m_positionArray[j].mData[2] * 0.1;
