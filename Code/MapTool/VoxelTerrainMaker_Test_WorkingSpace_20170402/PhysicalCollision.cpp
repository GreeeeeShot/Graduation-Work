...
void CPhysicalCollision::ImpurseBasedCollisionResolution(CPlayer *pPlayerA, CPlayer *pPlayerB)
{
	...
	pPlayerA->SetPosition(pPlayerA->GetPosition() - ((fRadiusA * d3dxvToB + fRadiusB * d3dxvToB) - fDistance * d3dxvToB));
	...
	D3DXVECTOR3 d3dxvJ = -(pPlayerA->m_d3dxvVelocity - pPlayerB->m_d3dxvVelocity) * (0.8f + 1.0f) * (PLAYER_MASS * PLAYER_MASS / (PLAYER_MASS + PLAYER_MASS));
