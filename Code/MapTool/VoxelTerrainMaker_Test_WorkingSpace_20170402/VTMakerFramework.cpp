	...
	for (int layer = 0; layer < (int)d3dxvWorkingSpaceSize.y; layer++)
	{
		for (int row = 0; row < (int)d3dxvWorkingSpaceSize.z; row++)
		{
			for (int col = 0; col < (int)d3dxvWorkingSpaceSize.x; col++)
			{
				...
				if (iSelectedMtrl = pWorkingSpace->m_iBlueprint[layer][row][col])
				{
					iLinearIdx = layer * pWorkingSpace->m_iLayerStride + row * pWorkingSpace->m_iRowStride + col;
					pWorkingSpace->m_ppVoxelObjects[iLinearIdx] = new CVoxel();
					...
					pWorkingSpace->m_ppVoxelObjects[iLinearIdx]->SetMesh(CMeshResource::ppRegisteredVoxelMesh[iSelectedMtrl-1]);
					pWorkingSpace->m_ppVoxelObjects[iLinearIdx]->SetTexture(CTextureResource::ppRegisteredTexture[iSelectedMtrl-1]);
