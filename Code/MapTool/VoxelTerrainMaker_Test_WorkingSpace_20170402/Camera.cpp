...
CCamera::CCamera()
{
	...
	D3DXMatrixIdentity(&m_d3dxmtxScreen);
...
void CCamera::SetViewport(ID3D11DeviceContext *pd3dDeviceContext, DWORD xTopLeft, DWORD yTopLeft, DWORD nWidth, DWORD nHeight, float fMinZ, float fMaxZ)
{
	...
	m_d3dxmtxScreen._11 = float(nWidth) / 2.0f;
	m_d3dxmtxScreen._22 = -float(nHeight) / 2.0f;
	m_d3dxmtxScreen._33 = fMaxZ - fMinZ;
	m_d3dxmtxScreen._41 = float(xTopLeft) + (float(nWidth) / 2.0f);
	m_d3dxmtxScreen._42 = float(yTopLeft) + (float(nHeight) / 2.0f);
...
D3DXMATRIX CCamera::GetScreenMatrix() const
{
	return m_d3dxmtxScreen;
}
