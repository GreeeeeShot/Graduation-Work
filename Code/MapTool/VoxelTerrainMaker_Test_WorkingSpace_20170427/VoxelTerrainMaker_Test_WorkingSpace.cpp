...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	gVTMakerFramework.OnCreate(hInstance, ghMainWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		EnableMenuItem(hSubMenu, IDM_EDIT_WORKINGSPACE_REVISE, MF_GRAYED);
		break;
	case WM_COMMAND:
	{
		...
		case IDM_FILE_OPEN:
			...
			if (GetOpenFileName(&OFN) != 0)
			{
				...
				if (answer == IDYES)
				{
					...
					hSubMenu = GetSubMenu(hMenu, 1);
					EnableMenuItem(hSubMenu, IDM_EDIT_WORKINGSPACE_REVISE, MF_ENABLED);
				}
			}
			break;
