...
void CVTMakerFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_MOUSEMOVE:
		if (m_pWorkingSpace && !bIsPaintButton)
		{
			if (bIsCreateButton)
			{
				m_pWorkingSpace->CreateVoxelObject(m_pd3dDevice, &m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			}
			else if (bIsDestroyButton)
			{
				m_pWorkingSpace->DestroyVoxelObject(m_pd3dDevice, &m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			}
			m_pWorkingSpace->Render(m_pd3dDeviceContext);
		}
		break;
	case WM_LBUTTONDOWN:
	...
	case WM_LBUTTONUP:
		if (m_pWorkingSpace && bIsPaintButton)
		{
			m_pWorkingSpace->CreateVoxelObjectByPaint(m_pd3dDevice, &m_CameraOperator.m_Camera, iMouseX, iMouseY, LOWORD(lParam), HIWORD(lParam));
		}
		iMouseX = 0;
		iMouseY = 0;
	...
	case WM_RBUTTONUP:
		if (m_pWorkingSpace && bIsPaintButton)
		{
			m_pWorkingSpace->DestroyVoxelObjectByPaint(m_pd3dDevice, &m_CameraOperator.m_Camera, iMouseX, iMouseY, LOWORD(lParam), HIWORD(lParam));
		}
		iMouseX = 0;
		iMouseY = 0;
...
void CVTMakerFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_UP:
			if (m_pWorkingSpace)
			{
				if(!bIsPaintButton) m_pWorkingSpace->MoveForwardVoxelTerrain();
				else m_pWorkingSpace->MoveUpVoxelTerrain();
			}
			break;
		case VK_DOWN:
			if (m_pWorkingSpace)
			{
				if (!bIsPaintButton) m_pWorkingSpace->MoveRearVoxelTerrain();
				else m_pWorkingSpace->MoveDownVoxelTerrain();
			}
			break;
		case VK_RIGHT:
			if (m_pWorkingSpace) m_pWorkingSpace->MoveRightVoxelTerrain();
			break;
		case VK_LEFT:
			if (m_pWorkingSpace) m_pWorkingSpace->MoveLeftVoxelTerrain();
			break;
		default:
			break;
		}
...
{
	if (!m_pWorkingSpace) CreateWorkingSpace(d3dxvWorkingSpaceSize, d3dxvCubeSize, d3dxvOffsetFromLocal);
	else m_pWorkingSpace->ReviseWorkingSpace(m_pd3dDevice, d3dxvWorkingSpaceSize, d3dxvCubeSize, d3dxvOffsetFromLocal);
}
