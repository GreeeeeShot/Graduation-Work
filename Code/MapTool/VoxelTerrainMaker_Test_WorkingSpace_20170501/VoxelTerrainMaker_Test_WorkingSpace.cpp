...
#define IDC_WORKINGSPACE_LIST_CTRL	5002
...
LRESULT CALLBACK FrameWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hWorkingSpaceList = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("listbox"), NULL,
			WS_VISIBLE | WS_CHILD | LBS_NOTIFY | WS_BORDER, FRAME_BUFFER_WIDTH + 50, 50, 300, 400, hWnd, (HMENU)IDC_WORKINGSPACE_LIST_CTRL, hInst, NULL);

		/*hActivateWorkingSpaceButton = CreateWindow(
			L"button",
			L"활성화",
			WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			FRAME_BUFFER_WIDTH + 150, 450, 100, 25, hWnd, (HMENU)IDC_ACTIVATE_WORKINGSPACE_BUTTON, hInst, NULL
		);*/
		/*GetClientRect(hWnd, &rectView);
		ghSupportWnd[0] = CreateWindowEx(
			WS_EX_CLIENTEDGE,
		...
		/*ghSupportWnd[1] = CreateWindowEx(
			...
			rectView.right / 2 + 1,
			0,
			rectView.right / 2, rectView.bottom,
			ghMainWnd,
			NULL,
			hInst,
			NULL
		...
		//printf("ghSupportWnd[0] [1]: %x, %x\n", ghSupportWnd[0], ghSupportWnd[1]);
		break;
	case WM_DROPFILES:
	{
	...
	case WM_COMMAND:
	{
		...
		case IDC_WORKINGSPACE_LIST_CTRL:
			if (HIWORD(wParam) == LBN_SELCHANGE)
			{
				...
				gVTMakerFramework.ChangeWorkingSpace((int)SendMessage(GetDlgItem(hWnd, IDC_WORKINGSPACE_LIST_CTRL), LB_GETCURSEL, 0, 0));
			}
		...
		case IDM_EDIT_MTRL_UPHILL_0:
			if (gVTMakerFramework.GetActivedWorkingSpace()) gVTMakerFramework.GetActivedWorkingSpace()->ChangeMaterial(VOXEL_TYPE_UPHILL_0);
			break;
		case IDM_EDIT_MTRL_UPHILL_1:
			if (gVTMakerFramework.GetActivedWorkingSpace()) gVTMakerFramework.GetActivedWorkingSpace()->ChangeMaterial(VOXEL_TYPE_UPHILL_1);
			break;
		case IDM_EDIT_MTRL_UPHILL_2:
			if (gVTMakerFramework.GetActivedWorkingSpace()) gVTMakerFramework.GetActivedWorkingSpace()->ChangeMaterial(VOXEL_TYPE_UPHILL_2);
			break;
		case IDM_EDIT_MTRL_UPHILL_3:
			if (gVTMakerFramework.GetActivedWorkingSpace()) gVTMakerFramework.GetActivedWorkingSpace()->ChangeMaterial(VOXEL_TYPE_UPHILL_3);
			break;
...
LRESULT CALLBACK    SupportWndProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		//printf("SupportKey");
		//if (ghSupportWnd[0]) gVTMakerFramework.OnProcessingWindowMessage(hDlg, iMsg, wParam, lParam);
		break;
	case WM_DESTROY:
		break;
	default:
