...
void CCameraOperator::MoveForward(float fDistancePerSec, float fTimeElapsed)				// 'W'
{
	D3DXVECTOR3 d3dxvMoving;

	d3dxvMoving = GetLook() * fDistancePerSec * fTimeElapsed;
	
	MoveCameraOperator(d3dxvMoving);
}

void CCameraOperator::MoveBack(float fDistancePerSec, float fTimeElapsed)					// 'S'
{
	D3DXVECTOR3 d3dxvMoving;
...
void CCameraOperator::MoveRight(float fDistancePerSec, float fTimeElapsed)					// 'D'
{
	D3DXVECTOR3 d3dxvMoving;

	d3dxvMoving = GetRight() * fDistancePerSec * fTimeElapsed;

	MoveCameraOperator(d3dxvMoving);
}

void CCameraOperator::MoveLeft(float fDistancePerSec, float fTimeElapsed)					// 'A'
{
	D3DXVECTOR3 d3dxvMoving;
...
void CCameraOperator::MoveUp(float fDistancePerSec, float fTimeElapsed)						// 'V'
{
	D3DXVECTOR3 d3dxvMoving;

	d3dxvMoving = GetUp() * fDistancePerSec * fTimeElapsed;

	MoveCameraOperator(d3dxvMoving);
}

void CCameraOperator::MoveDown(float fDistancePerSec, float fTimeElapsed)					// 'C'
{
	D3DXVECTOR3 d3dxvMoving;
...
void CCameraOperator::ZoomInAtOnce(float distance)						// 바라보고 있는 곳으로 가까워진다.
{
	...
	D3DXVec3Normalize(&m_d3dxvZoomDir, &D3DXVECTOR3(m_pObject? m_pObject->GetPosition() : m_d3dxvPosition - (m_d3dxvCameraDir * m_fDistance + m_d3dxvPosition)));
