		...
		for (int row = 0; row < (int)d3dxvWorkingSpaceSize.z; row++)
		{
			for (int col = 0; col < (int)d3dxvWorkingSpaceSize.x; col++)
			{
				...
				if (iSelectedMtrl = pWorkingSpace->m_iBlueprint[layer][row][col])
				{
					...
					pWorkingSpace->m_ppVoxelObjects[iLinearIdx]->SetMesh(CMeshResource::pVoxelMesh);
					pWorkingSpace->m_ppVoxelObjects[iLinearIdx]->SetTexture(CTextureResource::ppTextureMaterial[iSelectedMtrl-1]);
