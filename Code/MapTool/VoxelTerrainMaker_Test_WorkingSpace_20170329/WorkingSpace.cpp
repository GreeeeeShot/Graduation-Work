...
void CWorkingSpace::ChangeMaterial(int iMtrl)
{
	switch (iMtrl)
	{
	case TEXTURE_GRASS_0:
		m_iSelectedMtrl = TEXTURE_GRASS_0;
		break;
	case TEXTURE_WOOD_0:
		m_iSelectedMtrl = TEXTURE_WOOD_0;
		break;
	}
}
...
void CWorkingSpace::CreateVoxelObject(ID3D11Device *pd3dDevice, CCamera *pCamera, int iMouseX, int iMouseY)
{
	...
	m_ppVoxelObjects[iLinearIdx]->SetMesh(CMeshResource::pVoxelMesh);
	m_ppVoxelObjects[iLinearIdx]->SetTexture(CTextureResource::ppTextureMaterial[m_iSelectedMtrl]);
