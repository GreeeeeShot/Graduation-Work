...
CWorkingSpace::~CWorkingSpace()
{
	...
	if (m_pActivedWorkingSpace) delete m_pActivedWorkingSpace;
...
void CWorkingSpace::CreateWorkingSpace(ID3D11Device *pd3dDevice, D3DXVECTOR3 d3dxvWorkingSpaceSize, D3DXVECTOR3 d3dxvCubeSize, D3DXVECTOR3 d3dxvOffsetFromLocal)
{
	...
	m_pActivedWorkingSpace = new CGameObject();
	CMesh *pActivedWorkingSpace = new CDiffusedWorkingSpaceMesh(pd3dDevice, m_nMaxCol * m_fCubeWidth, m_nMaxLayer * m_fCubeHeight, m_nMaxRow * m_fCubeDepth, D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f));
	m_pActivedWorkingSpace->SetMesh(pActivedWorkingSpace);
	m_pActivedWorkingSpace->SetShader(CShaderResource::pDiffusedShader);
	m_pActivedWorkingSpace->SetPosition(m_fOffsetXFromLocal * m_fCubeWidth, m_fOffsetYFromLocal * m_fCubeHeight, m_fOffsetZFromLocal * m_fCubeDepth);
...
void CWorkingSpace::OptimizeWorkingSpace(ID3D11Device *pd3dDevice)
{
	D3DXVECTOR3 d3dxvMaxColLayerRow = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 d3dxvMinColLayerRow = D3DXVECTOR3(FLT_MAX, FLT_MAX, FLT_MAX);
	...
	for (int layer = 0; layer < m_nMaxLayer; layer++)
	{
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				if (m_ppVoxelObjects[layer * m_iLayerStride + row * m_iRowStride + col])
				{
					fDistanceX = (float)col;
					fDistanceY = (float)layer;
					fDistanceZ = (float)row;

					if (fDistanceX > d3dxvMaxColLayerRow.x) d3dxvMaxColLayerRow.x = fDistanceX;
					if (fDistanceY > d3dxvMaxColLayerRow.y) d3dxvMaxColLayerRow.y = fDistanceY;
					if (fDistanceZ > d3dxvMaxColLayerRow.z) d3dxvMaxColLayerRow.z = fDistanceZ;		
					if (fDistanceX < d3dxvMinColLayerRow.x) d3dxvMinColLayerRow.x = fDistanceX;
					if (fDistanceY < d3dxvMinColLayerRow.y) d3dxvMinColLayerRow.y = fDistanceY;
					if (fDistanceZ < d3dxvMinColLayerRow.z) d3dxvMinColLayerRow.z = fDistanceZ;
					bIsEmpty = false;
				}
			}
		}
	...
	/*for (int layer = 0; layer < m_nMaxLayer; layer++)
	{
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				if (m_ppVoxelObjects[layer * m_iLayerStride + row * m_iRowStride + col])
				{
					fDistanceX = (float)col - ((float)m_nMaxCol / 2.0f - 0.5f);
					fDistanceY = (float)layer + 1.0f;
					fDistanceZ = (float)row - ((float)m_nMaxRow / 2.0f - 0.5f);
	...
	d3dxvWorkingSpaceSize.x = (d3dxvMaxColLayerRow.x - d3dxvMinColLayerRow.x) + 1.0f;
	d3dxvWorkingSpaceSize.y = (d3dxvMaxColLayerRow.y - d3dxvMinColLayerRow.y) + 1.0f;
	d3dxvWorkingSpaceSize.z = (d3dxvMaxColLayerRow.z - d3dxvMinColLayerRow.z) + 1.0f;
	printf("fDistance : %f %f %f \n", d3dxvWorkingSpaceSize.x, d3dxvWorkingSpaceSize.y, d3dxvWorkingSpaceSize.z);
	m_d3dxmtxFromIdxToPos._41 = (d3dxvWorkingSpaceSize.x - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	// m_d3dxmtxFromIdxToPos._42 = -(m_iMaxLayer - 1) * m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._43 = -(d3dxvWorkingSpaceSize.z - 1.0f) * m_fCubeDepth / 2.0f + m_fOffsetZFromLocal;
	...
	D3DXVECTOR3 d3dxvIdx;
	int LinearIdx = -1;
	//d3dxvMaxColLayerRow -= d3dxvMinColLayerRow;
	//d3dxvMinColLayerRow -= d3dxvMinColLayerRow;

	int count = 0;
	for (int layer = d3dxvMinColLayerRow.y; layer < d3dxvMaxColLayerRow.y + 1; layer++)
	{
		for (int row = d3dxvMinColLayerRow.z; row < d3dxvMaxColLayerRow.z + 1; row++)
		{
			for (int col = d3dxvMinColLayerRow.x; col < d3dxvMaxColLayerRow.x + 1; col++)
			{
				if (m_ppVoxelObjects[LinearIdx = layer * m_iLayerStride + row * m_iRowStride + col])
				{
					count++;
					d3dxvIdx = D3DXVECTOR3(col - d3dxvMinColLayerRow.x, layer - d3dxvMinColLayerRow.y, row - d3dxvMinColLayerRow.z);
					ppTempVoxelObject[(int)(d3dxvIdx.y * d3dxvWorkingSpaceSize.x * d3dxvWorkingSpaceSize.z) + (int)(d3dxvIdx.z * d3dxvWorkingSpaceSize.x) + (int)d3dxvIdx.x] = m_ppVoxelObjects[LinearIdx];
					//d3dxvIdx = D3DXVECTOR3(col + j, layer, row + i);
					D3DXVec3Transform(&d3dxvPosition, &d3dxvIdx, &m_d3dxmtxFromIdxToPos);
					ppTempVoxelObject[(int)(d3dxvIdx.y * d3dxvWorkingSpaceSize.x * d3dxvWorkingSpaceSize.z) + (int)(d3dxvIdx.z * d3dxvWorkingSpaceSize.x) + (int)d3dxvIdx.x]->SetPosition(d3dxvPosition.x, d3dxvPosition.y, d3dxvPosition.z);
					//printf("Temp : (%d %d %d) : Voxel : (%d %d %d) \n", col + j, layer, row + i, j, layer, i);
				}
			}
		}
...
void CWorkingSpace::SaveWorkingSpaceToFile(FILE *pBluePrintFP)
{
	fprintf(pBluePrintFP, " %d %d %d %f %f %f %f %f %f \n",
	...
	for (int layer = 0; layer < m_nMaxLayer; layer++)
	{
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				if(m_ppVoxelObjects[iLinearIdx = layer * m_iLayerStride + row * m_iRowStride + col]) fprintf(pBluePrintFP, " %d", m_ppVoxelObjects[iLinearIdx]->m_eVoxelType);//m_iBlueprint[layer][row][col]);	//재질값을 입력받는다.
				else fprintf(pBluePrintFP, " %d", VOXEL_TYPE_NOT);
																																											 //printf("%d", m_iBlueprint[layer][row][col]);										//printf("%d", newbp->bluePrint[layer][breadth][length]);
			}
			fprintf(pBluePrintFP, " \n");
		}
		fprintf(pBluePrintFP, " \n");
	}
	fprintf(pBluePrintFP, " \n");
...
void CWorkingSpace::MoveUpWorkingSpace(void)
{
	m_fOffsetYFromLocal += m_fCubeHeight;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._43 = -(m_nMaxRow - 1.0f) * m_fCubeDepth / 2.0f + m_fOffsetZFromLocal;

	D3DXMatrixIdentity(&m_d3dxmtxFromPosToIdx);
	D3DXMatrixInverse(&m_d3dxmtxFromPosToIdx, NULL, &m_d3dxmtxFromIdxToPos);

	int iLinearIdx = 0;
	D3DXVECTOR4 d3dxvPos;
	D3DXVECTOR3 d3dxvIdx;
		...
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				if (m_ppVoxelObjects[iLinearIdx = layer * m_iLayerStride + row * m_iRowStride + col])
				{
					//d3dxvPos = m_ppVoxelObjects[iLinearIdx]->GetPosition();
					//m_ppVoxelObjects[iLinearIdx]->SetPosition(d3dxvPos.x, d3dxvPos.y + m_fCubeHeight, d3dxvPos.z);
					d3dxvIdx = D3DXVECTOR3(col, layer, row);
					D3DXVec3Transform(&d3dxvPos, &d3dxvIdx, &m_d3dxmtxFromIdxToPos);
					m_ppVoxelObjects[iLinearIdx]->SetPosition(d3dxvPos.x, d3dxvPos.y, d3dxvPos.z);
				}
			}
		}
	}
...
void CWorkingSpace::MoveDownWorkingSpace(void)
{
	m_fOffsetYFromLocal -= m_fCubeHeight;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
...
void CWorkingSpace::MoveForwardWorkingSpace(void)
{
	m_fOffsetZFromLocal += m_fCubeDepth;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._43 = -(m_nMaxRow - 1.0f) * m_fCubeDepth / 2.0f + m_fOffsetZFromLocal;
...
void CWorkingSpace::MoveRearWorkingSpace(void)
{
	m_fOffsetZFromLocal -= m_fCubeDepth;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
...
void CWorkingSpace::MoveRightWorkingSpace(void)
{
	m_fOffsetXFromLocal += m_fCubeWidth;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._43 = -(m_nMaxRow - 1.0f) * m_fCubeDepth / 2.0f + m_fOffsetZFromLocal;
...
void CWorkingSpace::MoveLeftWorkingSpace(void)
{
	m_fOffsetXFromLocal -= m_fCubeWidth;
	m_d3dxmtxFromIdxToPos._41 = (m_nMaxCol - 1.0f) * m_fCubeWidth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
...
void CWorkingSpace::RotateWorkingSpaceRight(ID3D11Device *pd3dDevice)
{
	D3DXMATRIX mtxRotate;

	D3DXMatrixRotationY(&mtxRotate, D3DXToRadian(90.0f));

	CVoxel** ppRotateVoxelObjects = new CVoxel*[m_nMaxLayer * m_nMaxRow * m_nMaxCol];
	ZeroMemory(ppRotateVoxelObjects, sizeof(CVoxel*) * (m_nMaxLayer * m_nMaxRow * m_nMaxCol));

	int iLinearIdx = 0;
	int iRotateLinearIdx = 0;
	D3DXVECTOR3 d3dxvRotatePos;

	D3DXVECTOR3 d3dxvCubeSize = D3DXVECTOR3(m_fCubeWidth, m_fCubeHeight, m_fCubeDepth);
	D3DXVECTOR3 d3dxvMaxColLayerRow = D3DXVECTOR3(m_nMaxCol, m_nMaxLayer, m_nMaxRow);
	D3DXVECTOR3 d3dxvOffsetFromLocal = D3DXVECTOR3(m_fOffsetXFromLocal, m_fOffsetYFromLocal, m_fOffsetZFromLocal);

	m_d3dxmtxFromIdxToPos._11 = -m_fCubeDepth;
	m_d3dxmtxFromIdxToPos._22 = m_fCubeHeight;
	m_d3dxmtxFromIdxToPos._33 = m_fCubeWidth;

	m_d3dxmtxFromIdxToPos._41 = (m_nMaxRow - 1) * m_fCubeDepth / 2.0f + m_fOffsetXFromLocal;
	m_d3dxmtxFromIdxToPos._42 = m_fCubeHeight / 2.0f + m_fOffsetYFromLocal;
	m_d3dxmtxFromIdxToPos._43 = -(m_nMaxCol - 1) * m_fCubeWidth / 2.0f + m_fOffsetZFromLocal;

	D3DXMatrixIdentity(&m_d3dxmtxFromPosToIdx);
	D3DXMatrixInverse(&m_d3dxmtxFromPosToIdx, NULL, &m_d3dxmtxFromIdxToPos);
	//D3DXVECTOR3 d3dxvTempRotatePos;
	...
	for (int layer = 0; layer < m_nMaxLayer; layer++)
	{
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				...
				{
					// Pos Revision
					d3dxvRotatePos = m_ppVoxelObjects[iLinearIdx]->GetPosition();
					d3dxvRotatePos -= d3dxvOffsetFromLocal;
					D3DXVec3TransformCoord(&d3dxvRotatePos, &d3dxvRotatePos, &mtxRotate);
					d3dxvRotatePos += d3dxvOffsetFromLocal;
					m_ppVoxelObjects[iLinearIdx]->SetPosition(d3dxvRotatePos);

					// Idx Revision
					D3DXVec3TransformCoord(&d3dxvRotatePos, &d3dxvRotatePos, &m_d3dxmtxFromPosToIdx);
					d3dxvRotatePos.x = roundf(d3dxvRotatePos.x);
					d3dxvRotatePos.y = roundf(d3dxvRotatePos.y);
					d3dxvRotatePos.z = roundf(d3dxvRotatePos.z);

					if (m_ppVoxelObjects[iLinearIdx]->m_bIsSlope)
					{
						eVoxelType = m_ppVoxelObjects[iLinearIdx]->m_eVoxelType;
						//eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_UPHILL_0 + 1) % (VOXEL_TYPE_UPHILL_3 - VOXEL_TYPE_UPHILL_0 + 1) + VOXEL_TYPE_UPHILL_0);
						if (VOXEL_TYPE_ZUNGLE_UPHILL_0 <= eVoxelType && eVoxelType <= VOXEL_TYPE_ZUNGLE_UPHILL_3) eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_ZUNGLE_UPHILL_0 + 1) % (VOXEL_TYPE_ZUNGLE_UPHILL_3 - VOXEL_TYPE_ZUNGLE_UPHILL_0 + 1) + VOXEL_TYPE_ZUNGLE_UPHILL_0);
						else if (VOXEL_TYPE_SNOW_UPHILL_0 <= eVoxelType && eVoxelType <= VOXEL_TYPE_SNOW_UPHILL_3) eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_SNOW_UPHILL_0 + 1) % (VOXEL_TYPE_SNOW_UPHILL_3 - VOXEL_TYPE_SNOW_UPHILL_0 + 1) + VOXEL_TYPE_SNOW_UPHILL_0);
						ppRotateVoxelObjects[iRotateLinearIdx = (int)d3dxvRotatePos.y * m_iLayerStride + (int)d3dxvRotatePos.z * m_nMaxRow + (int)d3dxvRotatePos.x]= CWorkingSpace::GetVoxelForType((VOXEL_TYPE)eVoxelType);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetShader(CShaderResource::pTexturedLightingShader);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetMaterial(CMaterialResource::pStandardMaterial);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetPosition(m_ppVoxelObjects[iLinearIdx]->GetPosition());
						delete m_ppVoxelObjects[iLinearIdx];
					}
					else ppRotateVoxelObjects[(int)d3dxvRotatePos.y * m_iLayerStride + (int)d3dxvRotatePos.z * m_nMaxRow + (int)d3dxvRotatePos.x] = m_ppVoxelObjects[iLinearIdx];
				}
			}
		}
	}
	...
	//m_nMaxLayer = d3dxvWorkingSpaceSize.y;
	m_nMaxCol = d3dxvMaxColLayerRow.z;
	m_nMaxRow = d3dxvMaxColLayerRow.x;
	m_fCubeWidth = d3dxvCubeSize.z;
	//m_fCubeHeight = d3dxvCubeSize.y;
	m_fCubeDepth = d3dxvCubeSize.x;
	...
	m_ppVoxelObjects = ppRotateVoxelObjects;
...
void CWorkingSpace::RotateWorkingSpaceLeft(ID3D11Device *pd3dDevice)
{
	D3DXMATRIX mtxRotate;
		...
		for (int row = 0; row < m_nMaxRow; row++)
		{
			for (int col = 0; col < m_nMaxCol; col++)
			{
				if (m_ppVoxelObjects[iLinearIdx = layer * m_iLayerStride + row * m_iRowStride + col])
				{
					...
					{
						eVoxelType = m_ppVoxelObjects[iLinearIdx]->m_eVoxelType;
						//eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_UPHILL_0 + (VOXEL_TYPE_UPHILL_3 - VOXEL_TYPE_UPHILL_0 + 1) - 1) % (VOXEL_TYPE_UPHILL_3 - VOXEL_TYPE_UPHILL_0 + 1) + VOXEL_TYPE_UPHILL_0);
						if (VOXEL_TYPE_ZUNGLE_UPHILL_0 <= eVoxelType && eVoxelType <= VOXEL_TYPE_ZUNGLE_UPHILL_3) eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_ZUNGLE_UPHILL_0 + (VOXEL_TYPE_ZUNGLE_UPHILL_3 - VOXEL_TYPE_ZUNGLE_UPHILL_0 + 1) - 1) % (VOXEL_TYPE_ZUNGLE_UPHILL_3 - VOXEL_TYPE_ZUNGLE_UPHILL_0 + 1) + VOXEL_TYPE_ZUNGLE_UPHILL_0);
						else if (VOXEL_TYPE_SNOW_UPHILL_0 <= eVoxelType && eVoxelType <= VOXEL_TYPE_SNOW_UPHILL_3) eVoxelType = (VOXEL_TYPE)((m_ppVoxelObjects[iLinearIdx]->m_eVoxelType - VOXEL_TYPE_SNOW_UPHILL_0 + (VOXEL_TYPE_SNOW_UPHILL_3 - VOXEL_TYPE_SNOW_UPHILL_0 + 1) - 1) % (VOXEL_TYPE_SNOW_UPHILL_3 - VOXEL_TYPE_SNOW_UPHILL_0 + 1) + VOXEL_TYPE_SNOW_UPHILL_0);
						ppRotateVoxelObjects[iRotateLinearIdx = (int)d3dxvRotatePos.y * m_iLayerStride + (int)d3dxvRotatePos.z * m_nMaxRow + (int)d3dxvRotatePos.x] = CWorkingSpace::GetVoxelForType((VOXEL_TYPE)eVoxelType);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetShader(CShaderResource::pTexturedLightingShader);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetMaterial(CMaterialResource::pStandardMaterial);
						ppRotateVoxelObjects[iRotateLinearIdx]->SetPosition(m_ppVoxelObjects[iLinearIdx]->GetPosition());
...
void CWorkingSpace::CreateVoxelObject(ID3D11Device *pd3dDevice, CCamera *pCamera, int iMouseX, int iMouseY)
{
	...
	AABB MovedAABB =
		CPhysicalCollision::MoveAABB(
			...
			D3DXVECTOR3(m_pWorkingBoard->GetPosition().x, m_pWorkingBoard->GetPosition().y, m_pWorkingBoard->GetPosition().z));
	...
	m_ppVoxelObjects[iLinearIdx] = CWorkingSpace::GetVoxelForType((VOXEL_TYPE)m_iSelectedMtrl);
...
void CWorkingSpace::Render(ID3D11DeviceContext *pd3dDeviceContext, bool bIsActived)
{
	...
	if (bIsActived)
	{
		if (m_pActivedWorkingSpace) m_pActivedWorkingSpace->Render(pd3dDeviceContext);
	}
	else
	{
