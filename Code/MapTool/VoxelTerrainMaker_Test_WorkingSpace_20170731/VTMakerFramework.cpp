...
bool bIsSupportButton = false;
bool bIsShiftButton = false;
...
{
	//printf("msg : %x ", nMessageID);
	switch (nMessageID)
	{
	case WM_MOUSEMOVE:
		if (!m_vecWorkingSpace.empty() && !bIsSupportButton)
		{
			if (bIsCreateButton)
			{
				
				m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->CreateVoxelObject(m_pd3dDevice, &m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			}
			else if (bIsDestroyButton)
			{
				m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->DestroyVoxelObject(m_pd3dDevice, &m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			}
			m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->Render(m_pd3dDeviceContext, true);
		}
		break;
	case WM_LBUTTONDOWN:
	...
	case WM_LBUTTONUP:
		if (!m_vecWorkingSpace.empty() && bIsSupportButton)
		{
			m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->CreateVoxelObjectByPaint(m_pd3dDevice, &m_CameraOperator.m_Camera, iMouseX, iMouseY, LOWORD(lParam), HIWORD(lParam));
		}
		iMouseX = 0;
		iMouseY = 0;
	...
	case WM_RBUTTONUP:
		if (!m_vecWorkingSpace.empty() && bIsSupportButton)
		{
			m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->DestroyVoxelObjectByPaint(m_pd3dDevice, &m_CameraOperator.m_Camera, iMouseX, iMouseY, LOWORD(lParam), HIWORD(lParam));
		}
		iMouseX = 0;
		iMouseY = 0;
...
void CVTMakerFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		{
		case VK_SPACE:
			bIsSupportButton = false;
			break;
		/*case VK_SHIFT:
			bIsShiftButton = false;
			break;*/
		default:
			break;
		}
	...
	case WM_KEYDOWN:
		...
		case VK_TAB:
			...
			break;
		case VK_SPACE:
			bIsSupportButton = true;
			break;
		case VK_SHIFT:
			bIsShiftButton = bIsShiftButton? false : true;
			break;
		case VK_UP:
			if (!m_vecWorkingSpace.empty())
			{
				if (bIsShiftButton)
				{
					if (!bIsSupportButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveForwardWorkingSpace();
					else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveUpWorkingSpace();
				}
				else
				{
					if (!bIsSupportButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveForwardVoxelTerrain();
					else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveUpVoxelTerrain();
				}
			}
			break;
		case VK_DOWN:
			if (!m_vecWorkingSpace.empty())
			{
				if (bIsShiftButton)
				{
					if (!bIsSupportButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveRearWorkingSpace();
					else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveDownWorkingSpace();
				}
				else
				{
					if (!bIsSupportButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveRearVoxelTerrain();
					else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveDownVoxelTerrain();
				}
			}
			break;
		case VK_RIGHT:
			if (!m_vecWorkingSpace.empty())
			{
				if (bIsShiftButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveRightWorkingSpace();
				else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveRightVoxelTerrain();
			}
			break;
		case VK_LEFT:
			if (!m_vecWorkingSpace.empty())
			{
				if (bIsShiftButton) m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveLeftWorkingSpace();
				else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->MoveLeftVoxelTerrain();
			}
			break;
		default:
			break;
...
void CVTMakerFramework::PushWorkingSpace(CWorkingSpace *pWorkingSpace)
{
	/*if (!m_vecWorkingSpace.empty())
	{
		delete m_vecWorkingSpace[m_iActivedWorkingSpaceIdx];
		m_vecWorkingSpace.erase(m_vecWorkingSpace.begin() + m_iActivedWorkingSpaceIdx);
		if(m_iActivedWorkingSpaceIdx > 0) m_iActivedWorkingSpaceIdx--;
		m_iWorkingSpaceNum--;
	}*/
	//m_pWorkingSpace[m_iActivedWorkingSpaceIdx] = pWorkingSpace;
	m_vecWorkingSpace.push_back(pWorkingSpace);
	m_iWorkingSpaceNum++;
	m_iActivedWorkingSpaceIdx = m_iWorkingSpaceNum - 1;
}

void CVTMakerFramework::EraseActivedWorkingSpace(void)
{
	if (!m_vecWorkingSpace.empty())
	{
		delete m_vecWorkingSpace[m_iActivedWorkingSpaceIdx];
...
void CVTMakerFramework::CreateWorkingSpace(D3DXVECTOR3 d3dxvWorkingSpaceSize, D3DXVECTOR3 d3dxvCubeSize, D3DXVECTOR3 d3dxvOffsetFromLocal)
{
	...
	PushWorkingSpace(pWorkingSpace);
}

void CVTMakerFramework::TransformWorkingSpace(D3DXVECTOR3 d3dxvWorkingSpaceSize, D3DXVECTOR3 d3dxvCubeSize, D3DXVECTOR3 d3dxvOffsetFromLocal)
{
	if (!m_vecWorkingSpace.empty())
	{
		if (!m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]) CreateWorkingSpace(d3dxvWorkingSpaceSize, d3dxvCubeSize, d3dxvOffsetFromLocal);
		else m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->ReviseWorkingSpace(m_pd3dDevice, d3dxvWorkingSpaceSize, d3dxvCubeSize, d3dxvOffsetFromLocal);
	}
	
}

void CVTMakerFramework::RegenerateWorkingSpace(D3DXVECTOR3 d3dxvWorkingSpaceSize, D3DXVECTOR3 d3dxvCubeSize, D3DXVECTOR3 d3dxvOffsetFromLocal)
{
	CWorkingSpace *pWorkingSpace = new CWorkingSpace();

	pWorkingSpace->CreateWorkingSpace(m_pd3dDevice, d3dxvWorkingSpaceSize, d3dxvCubeSize, d3dxvOffsetFromLocal);
	EraseActivedWorkingSpace();
	//m_pWorkingSpace[m_iActivedWorkingSpaceIdx] = pWorkingSpace;
	PushWorkingSpace(pWorkingSpace);
}
void CVTMakerFramework::DestroyWorkingSpace(void)
{
	EraseActivedWorkingSpace();
}
void CVTMakerFramework::OptimizeWorkingSpace(void)
{
	if (!m_vecWorkingSpace.empty())
	{
		//if (!m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]) return;
		m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->OptimizeWorkingSpace(m_pd3dDevice);
	}
}

void CVTMakerFramework::CreateWorkingSpaceFromFile(HWND hList, const char* pFileName)
{
	FILE* bluePrintFp;
	...
	fscanf_s(bluePrintFp, " %d", &iMapTotalNum);

	for (int i = 0; i < iMapTotalNum; i++)
	{
		...
		char str[30] = "작업공간";
		char num[5];
		itoa(m_iActivedWorkingSpaceIdx, num, 10);
		strcat(str, num);
		CHAR *strMB = str;
		WCHAR strWC[1024];
		MultiByteToWideChar(CP_ACP, 0, strMB, -1, strWC, 1024);
		SendMessage(hList, LB_ADDSTRING, m_iActivedWorkingSpaceIdx, (LPARAM)strWC);
	}

	fclose(bluePrintFp);
}

void CVTMakerFramework::SaveActivedWorkingSpaceToFile(const char* pFileName)
{
	if (!m_vecWorkingSpace.empty())
	{
		FILE *pBluePrintFP;

		pBluePrintFP = fopen(pFileName, "w");

		if (pBluePrintFP == NULL)
		{
			exit(1);
		}

		fprintf(pBluePrintFP, " %d \n", 1);

		if (!m_vecWorkingSpace.empty()) GetActivedWorkingSpace()->SaveWorkingSpaceToFile(pBluePrintFP);

		fclose(pBluePrintFP);
	}
}

void CVTMakerFramework::SaveAllWorkingSpaceToFile(const char* pFileName)
{
	if (!m_vecWorkingSpace.empty())
	{
		FILE *pBluePrintFP;
		...
		fprintf(pBluePrintFP, " %d \n", m_vecWorkingSpace.size());

		for (int i = 0; i < m_vecWorkingSpace.size(); i++)
		{
			m_vecWorkingSpace[i]->SaveWorkingSpaceToFile(pBluePrintFP);
		}
...
void CVTMakerFramework::SaveIndividualWorkingSpaceToFile(const char* pFileName)
{
	if (!m_vecWorkingSpace.empty())
	{
		FILE *pBluePrintFP;
		...
		for (i = 0; i < strlen(pFileName); i++)
		{
			if (!bIsExtension)
			{
				if (pFileName[i] == '.')
				{
					bIsExtension = true;
					fileName[i] = '\0';
					fileNameExtension[i - fileNameCount] = '.';
				}
				else
				{
			...
			{
				fileNameExtension[i - fileNameCount] = pFileName[i];
			}
		}
		fileNameExtension[i - fileNameCount] = '\0';

		for (i = 0; i < m_vecWorkingSpace.size(); i++)
		{
			itoa(i, num, 10);
			strcat(fileName, num);
			...
			fprintf(pBluePrintFP, " %d \n", 1);
			m_vecWorkingSpace[i]->SaveWorkingSpaceToFile(pBluePrintFP);
			fileName[strlen(fileName) - (strlen(num) + strlen(fileNameExtension))] = '\0';
			fclose(pBluePrintFP);
		}
	}
}
...
void CVTMakerFramework::ReleaseObjects()
{
	for (int i = 0; i < m_vecWorkingSpace.size(); i++)
	{
		if (m_vecWorkingSpace[i]) delete m_vecWorkingSpace[i];
	}
	m_vecWorkingSpace.clear();
...
{
	if (0 <= iIdx && iIdx < m_vecWorkingSpace.size())
	{
		m_iActivedWorkingSpaceIdx = iIdx;
	}
}

void CVTMakerFramework::SetToStampWoringSpace(int iIdx)
{
	if (0 <= iIdx && iIdx < m_vecWorkingSpace.size())
	{
		m_iToStampWorkingSpaceIdx = iIdx;
	}
}

void CVTMakerFramework::Stamp(void)
{
	if (m_vecWorkingSpace.empty()) return;

	int nMaxColOfTo = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_nMaxCol;
	int nMaxLayerOfTo = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_nMaxLayer;
	int nMaxRowOfTo = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_nMaxRow;

	int iLayerStrideOfTo = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_iLayerStride;
	int iRowStrideOfTo = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_iRowStride;

	int nMaxColOfFrom = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_nMaxCol;
	int nMaxLayerOfFrom = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_nMaxLayer;
	int nMaxRowOfFrom = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_nMaxRow;

	int iLayerStrideOfFrom = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_iLayerStride;
	int iRowStrideOfFrom = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_iRowStride;

	D3DXVECTOR4 d3dxvColLayerRow;
	D3DXMATRIX d3dxmtxFromPosToIdx = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_d3dxmtxFromPosToIdx;
	D3DXMATRIX d3dxmtxFromIdxToPos = m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_d3dxmtxFromIdxToPos;

	int iLinearIdx;
	CVoxel* stampedVoxel;

	for (int layer = 0; layer < nMaxLayerOfFrom; layer++)
	{
		for (int row = 0; row < nMaxRowOfFrom; row++)
		{
			for (int col = 0; col < nMaxColOfFrom; col++)
			{
				if (stampedVoxel = m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx = layer * iLayerStrideOfFrom + row * iRowStrideOfFrom + col])
				{
					D3DXVec3Transform(&d3dxvColLayerRow, &stampedVoxel->GetPosition(), &d3dxmtxFromPosToIdx);
					...
					if ((0 <= d3dxvColLayerRow.x && d3dxvColLayerRow.x < nMaxColOfTo)
						&& (0 <= d3dxvColLayerRow.y && d3dxvColLayerRow.y < nMaxLayerOfTo)
						&& (0 <= d3dxvColLayerRow.z && d3dxvColLayerRow.z < nMaxRowOfTo))
					{
						if (m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx = (int)d3dxvColLayerRow.y * iLayerStrideOfTo + (int)d3dxvColLayerRow.z * iRowStrideOfTo + (int)d3dxvColLayerRow.x])
						{
							delete m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx];
							m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx] = NULL;
						}
						//printf("ddsdfsf");
						m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx] = CWorkingSpace::GetVoxelForType(stampedVoxel->m_eVoxelType);
						m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx]->SetShader(CShaderResource::pTexturedLightingShader);
						m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx]->SetMaterial(CMaterialResource::pStandardMaterial);
						D3DXVec3Transform(&d3dxvColLayerRow, &D3DXVECTOR3(d3dxvColLayerRow.x, d3dxvColLayerRow.y, d3dxvColLayerRow.z), &d3dxmtxFromIdxToPos);
						m_vecWorkingSpace[m_iToStampWorkingSpaceIdx]->m_ppVoxelObjects[iLinearIdx]->SetPosition(d3dxvColLayerRow.x, d3dxvColLayerRow.y, d3dxvColLayerRow.z);
					}
				}
			}
		}
...
void CVTMakerFramework::RotateWorkingSpaceRight()
{
	if (!m_vecWorkingSpace.empty())
	{
		m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->RotateWorkingSpaceRight(m_pd3dDevice);
	}
}
void CVTMakerFramework::RotateWorkingSpaceLeft()
{
	if (!m_vecWorkingSpace.empty())
	{
		m_vecWorkingSpace[m_iActivedWorkingSpaceIdx]->RotateWorkingSpaceLeft(m_pd3dDevice);
	}
}
...
void CVTMakerFramework::FrameAdvance()
{
	...
	for (int i = 0; i < m_vecWorkingSpace.size(); i++)
	{
		...
		if (m_vecWorkingSpace[i]) m_vecWorkingSpace[i]->Render(m_pd3dDeviceContext, bIsActived);
	}
	//printf("render");
