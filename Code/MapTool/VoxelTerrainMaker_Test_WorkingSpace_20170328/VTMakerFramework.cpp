...
void CVTMakerFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	static CShader *pTexturedLightingShader = new CTexturedLightingShader();
	static bool bIsCreated = false;
	
	//D3DXVECTOR3 d3dxvRayVector;
	CBlueprint *bp;
	CVoxelTerrain *pVoxelTerrain;

	if (!bIsCreated)
	{
		pTexturedLightingShader->CreateShader(m_pd3dDevice);
		printf("pTexturedLightingShader : %x \n", pTexturedLightingShader);
		bIsCreated = true;
	}
	...
	case WM_MOUSEMOVE:
		...
		if (m_pWorkingSpace != NULL)// && bDrag)
		{
			//d3dxvRayVector = m_pWorkingSpace->CreateRayVector(&m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			//printf("d3dxvRayVector(%f, %f, %f) \n", d3dxvRayVector.x, d3dxvRayVector.y, d3dxvRayVector.z);
		...
		break;
	case WM_LBUTTONDOWN:
		m_pWorkingSpace->CreateBlueprint();
		
		m_pWorkingSpace->CreateVoxelObject(m_pd3dDevice, &m_CameraOperator.m_Camera, LOWORD(lParam), HIWORD(lParam));
			
		pVoxelTerrain = new CVoxelTerrain();
		bp = m_pWorkingSpace->CreateBlueprint();
		pVoxelTerrain->CreateVoxelTerrain(bp, pTexturedLightingShader);SetVoxelTerrain(pVoxelTerrain);
		delete bp;
		bDrag = true;
		break;
