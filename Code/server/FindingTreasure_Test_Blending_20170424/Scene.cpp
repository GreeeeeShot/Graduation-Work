...
void CScene::BuildObjects(ID3D11Device *pd3dDevice)
{
	...
	m_pVoxelTerrain->CreateVoxelTerrain(pTestMapBP->LoadBlueprint("test_map2.txt"));
...
void CScene::InitScene(void)
{
	...
	m_pTreasureChest->SetPosition(0.0f, 10.f, 0.0f);
	m_ppPlayers[0]->InitPlayer(D3DXVECTOR3(-7.0f, 40.0f, 7.0f));
...
bool CScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		m_ppPlayers[0]->m_bIsPushed = false;
		break;
	case WM_KEYDOWN:
		switch (wParam)
		{
		case 'a': case 'A':
			if (!m_ppPlayers[0]->m_bIsPushed)
			{
				m_ppPlayers[0]->PushPlayer(m_pTreasureChest);
				m_ppPlayers[0]->m_bIsPushed = true;
			}
			break;
		case '1': case '2': case '3': case '4': case '5':
			if (!m_ppPlayers[0]->m_bIsDigOrInstall) m_ppPlayers[0]->m_iVoxelPocketSlotIdx = wParam - '0' - 1;
...
void CScene::MoveObjectUnderPhysicalEnvironment(CPlayer *pPlayer, float fTimeElapsed)
{
	...
	D3DXVECTOR3 d3dxvCollisionPivot = D3DXVECTOR3(pPlayer->GetPosition().x, pPlayer->GetPosition().y + pPlayer->m_pMesh->m_AABB.m_d3dxvMin.y + 0.025f, pPlayer->GetPosition().z);
	...
	if (!bIsFloor)			// 복셀과 충돌한 것이 아니라면
	{
		for (int i = 0; i < 2; i++)
		{
				...
				&CPhysicalCollision::MoveAABB(m_pShips[i]->m_pMesh->m_AABB, m_pShips[i]->GetPosition())))
			{
				...
				if (m_ppPlayers[0]->GetPosition().y > m_pShips[i]->GetPosition().y + m_pShips[i]->m_pMesh->m_AABB.m_d3dxvMax.y)
				{

					D3DXVECTOR3 d3dxvRevision = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
					d3dxvRevision.y = m_pShips[i]->GetPosition().y + m_pShips[i]->m_pMesh->m_AABB.m_d3dxvMax.y - (m_ppPlayers[0]->GetPosition().y + m_ppPlayers[0]->m_pMesh->m_AABB.m_d3dxvMin.y);
					m_ppPlayers[0]->Moving(d3dxvRevision);
					m_ppPlayers[0]->m_CameraOperator.MoveCameraOperator(d3dxvRevision);
...
void CScene::AnimateObjects(ID3D11DeviceContext*pd3dDeviceContext, float fTimeElapsed)
{
	...
	if (m_ppPlayers[0]->m_bIsActive)
	{
		m_ppPlayers[0]->RotateMoveDir(fTimeElapsed);

		if (CPhysicalCollision::IsCollided(
			&CPhysicalCollision::MoveAABB(m_ppPlayers[0]->m_pMesh->m_AABB, m_ppPlayers[0]->GetPosition()),
			&CPhysicalCollision::MoveAABB(m_pTreasureChest->m_pMesh->m_AABB, m_pTreasureChest->GetPosition())))
		{
			if (m_ppPlayers[0]->m_d3dxvVelocity.y <= 0.0f && m_ppPlayers[0]->GetPosition().y > m_pTreasureChest->GetPosition().y + m_pTreasureChest->m_pMesh->m_AABB.m_d3dxvMax.y)
			{
				//if (pPlayer->m_d3dxvVelocity.y > 0.0f) 
				D3DXVECTOR3 d3dxvRevision = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
				d3dxvRevision.y = m_pTreasureChest->GetPosition().y + m_pTreasureChest->m_pMesh->m_AABB.m_d3dxvMax.y - (m_ppPlayers[0]->GetPosition().y + m_ppPlayers[0]->m_pMesh->m_AABB.m_d3dxvMin.y);
				m_ppPlayers[0]->Moving(d3dxvRevision);
				m_ppPlayers[0]->m_CameraOperator.MoveCameraOperator(d3dxvRevision);
				m_ppPlayers[0]->m_d3dxvVelocity.y = 0.0;
				if (m_ppPlayers[0]->m_d3dxvMoveDir.y != 0.0f) m_ppPlayers[0]->m_d3dxvVelocity.y = 4.95;
			}
			else CPhysicalCollision::ImpurseBasedCollisionResolution(m_ppPlayers[0], m_pTreasureChest);
		}

		MoveObjectUnderPhysicalEnvironment(m_ppPlayers[0], fTimeElapsed);

		if (m_ppPlayers[0]->GetPosition().y < -1.0f)
		{
			printf("플레이어가 물에 빠져버렸습니다!");
			m_RespawnManager.RegisterRespawnManager(m_ppPlayers[0]);
		}
	}
