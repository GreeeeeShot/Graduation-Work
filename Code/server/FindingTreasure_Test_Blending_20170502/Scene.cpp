...
void CScene::BuildObjects(ID3D11Device *pd3dDevice)
{
	...
	m_pVoxelTerrain->CreateVoxelTerrain(pTestMapBP->LoadBlueprint("test_map4.txt"));
...
void CScene::AnimateObjects(ID3D11DeviceContext*pd3dDeviceContext, float fTimeElapsed)
{
	...
	for (int i = 0; i < m_pPlayersMgrInform->m_iPlayersNum; i++)
	{
		...
		{
			if (m_pPlayersMgrInform->m_ppPlayers[i]->move)
				m_pPlayersMgrInform->m_ppPlayers[i]->FramePerMove();
