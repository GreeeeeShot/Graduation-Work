...
void CPlayer::PushPlayer(CPlayer *pPlayer)
{
	...
	else if (pPlayer)
	{
		...
		pPlayer->m_d3dxvVelocity.x += d3dxvPushVelocity.x * 50.0f;
		pPlayer->m_d3dxvVelocity.z += d3dxvPushVelocity.z * 50.0f;
	}
}
...
void CPlayer::PushPlayers(CPlayer **ppPlayers, int nConnectedPlayersNum)
{
	...
	for (int i = 0; i < nConnectedPlayersNum; i++)
	{
		...
		else if (ppPlayers[i])
		{
			...
			ppPlayers[i]->m_d3dxvVelocity.x += d3dxvPushVelocity.x * 50.0f;
			ppPlayers[i]->m_d3dxvVelocity.z += d3dxvPushVelocity.z * 50.0f;;
		}
	}
}
...
void CPlayer::SetMove(float x, float y, float z, float t)
{
	move = true;
...
void CPlayer::FramePerMove()
{
	...
	if ((m_Vel.x - GetPosition().x)*(m_Vel.x - GetPosition().x) < 0.1&&(m_Vel.y - GetPosition().y)*(m_Vel.y - GetPosition().y) < 0.1&&(m_Vel.z - GetPosition().z)*(m_Vel.z - GetPosition().z) < 0.1)
	{
