...
float CVoxel::GetHeightByPlayerPos(D3DXVECTOR3 d3dxvPlayerPos)
{
	...
	*/
		//printf("fU %f, fV %f \n", fU, fV);
		/*if (fU > 1.0f) fU = 1.0f;
		else if (fU < 0.0f) fU = 0.0f;
		if (fV > 1.0f) fV = 1.0f;
		else if (fV < 0.0f) fV = 0.0f;*/
...
void CVoxel::Render(ID3D11DeviceContext *pd3dDeviceContext, CShader* pShader)
{
	if (pShader)
	{
		pShader->UpdateShaderVariable(pd3dDeviceContext, &m_d3dxmtxWorld);
		if (m_pMaterial) pShader->UpdateShaderVariable(pd3dDeviceContext, &m_pMaterial->m_Material);
		//if (m_pTexture) pShader->UpdateShaderVariables(pd3dDeviceContext, m_pTexture);
