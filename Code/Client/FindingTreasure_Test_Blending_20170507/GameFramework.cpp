...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		m_pPlayersMgrInform->GetMyPlayer()->m_CameraOperator.GenerateProjectionMatrix(1.0f, 500.0f, float(m_nWndClientWidth) / float(m_nWndClientHeight), 90.0f);
...
void CGameFramework::BuildObjects()
{
	...
	m_pScene = new CScene("new mapfile test/jungle.txt");
	//플레이어 객체를 생성한다.
	...
	m_pPlayersMgrInform->m_iMyPlayerID = 0;
	...
	for (int i = 0; i <m_pPlayersMgrInform->m_iPlayersNum; i++)
	{
		...
		m_pPlayersMgrInform->m_ppPlayers[i]->SetMesh(CMeshResource::pPirateMesh);
		m_pPlayersMgrInform->m_ppPlayers[i]->SetShader(CShaderResource::pTexturedLightingShader);
		m_pPlayersMgrInform->m_ppPlayers[i]->SetTexture(CTextureResource::pGrassTexture);
...
void CGameFramework::ProcessInput()
{
	...
	if (m_pPlayersMgrInform->GetMyPlayer()->m_bIsActive)
	{
		if (GetKeyboardState(pKeyBuffer))
		{
			...
			{
				if (pKeyBuffer[VK_SPACE] & 0xF0)	pMyPlayer->m_d3dxvMoveDir.y = 5.0f;
				if (pKeyBuffer[VK_UP] & 0xF0)		pMyPlayer->m_d3dxvMoveDir.z += 2.0f;
				if (pKeyBuffer[VK_DOWN] & 0xF0)		pMyPlayer->m_d3dxvMoveDir.z -= 2.0f;
				if (pKeyBuffer[VK_LEFT] & 0xF0)		pMyPlayer->m_d3dxvMoveDir.x -= 2.0f;
				if (pKeyBuffer[VK_RIGHT] & 0xF0)	pMyPlayer->m_d3dxvMoveDir.x += 2.0f;
...
void CGameFramework::FrameAdvance()
{
	...
	AnimateObjects();
	CEffectManager::RenderEffects(m_pd3dDeviceContext);
	if (m_pUI)
	{
		if (m_pd3dDepthStencilView) m_pd3dDeviceContext->ClearDepthStencilView(m_pd3dDepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
