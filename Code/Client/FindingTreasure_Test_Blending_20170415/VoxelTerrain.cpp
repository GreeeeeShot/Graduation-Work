...
bool CVoxelTerrain::IsInnerVoxel(CVoxel* pVoxel)
{
	...
	D3DXVec3Transform(&d3dxvColLayerRow, &GetPosition(), &m_d3dxmtxFromPosToIdx);
...
void CVoxelTerrain::Render(ID3D11DeviceContext *pd3dDeviceContext, CCamera *pCamera)
{
	...
	for (int i = 1; i < VOXEL_TYPE_NUM; i++)
	{
		...
		for (int j = 0; j < size; j++)
		{
			...
			{
				if (!IsInnerVoxel(m_vecVoxelObjectsForRendering[i][j]))
				{
