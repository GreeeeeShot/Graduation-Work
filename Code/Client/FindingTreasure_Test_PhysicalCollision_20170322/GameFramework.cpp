...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetMaterial(pStandardMaterial);
	m_pPlayer->SetMesh(CMeshResource::pPirateMesh);
	m_pPlayer->SetShader(pTexturedLightingShader);
	...
	m_pPlayer->SetTexture(pTexture);
	m_pPlayer->SetPosition(0.0f, 40.0f, 0.0f);
...
void CGameFramework::SetPlayerToScene(CScene *pScene, CPlayer *pPlayer)
{
...
void CGameFramework::ProcessInput()
{
	...
	D3DXVECTOR3 d3dxvDirSum = D3DXVECTOR3(0.0f, 0.0f, 0.0f);

	if (GetKeyboardState(pKeyBuffer))
	{
		float fTimeElapsed = m_GameTimer.GetTimeElapsed();
		if (pKeyBuffer['R'] & 0xF0)			m_pPlayer->m_CameraOperator.RotateLocalX(CAMERA_ROTATION_DEGREE_PER_SEC, fTimeElapsed);
		if (pKeyBuffer['F'] & 0xF0)			m_pPlayer->m_CameraOperator.RotateLocalX(-CAMERA_ROTATION_DEGREE_PER_SEC, fTimeElapsed);
		if (pKeyBuffer['Q'] & 0xF0)			m_pPlayer->m_CameraOperator.RotateLocalY(-CAMERA_ROTATION_DEGREE_PER_SEC, fTimeElapsed);
		if (pKeyBuffer['E'] & 0xF0)			m_pPlayer->m_CameraOperator.RotateLocalY(CAMERA_ROTATION_DEGREE_PER_SEC, fTimeElapsed);
		if (pKeyBuffer[VK_UP] & 0xF0)		{ d3dxvDirSum.z += 2.0f; m_pPlayer->GoForward(fTimeElapsed); }
		if (pKeyBuffer[VK_DOWN] & 0xF0)		{ d3dxvDirSum.z -= 2.0f; m_pPlayer->GoBack(fTimeElapsed); }
		if (pKeyBuffer[VK_LEFT] & 0xF0)		{ d3dxvDirSum.x -= 2.0f; m_pPlayer->GoLeft(fTimeElapsed); }
		if (pKeyBuffer[VK_RIGHT] & 0xF0)	{ d3dxvDirSum.x += 2.0f; m_pPlayer->GoRight(fTimeElapsed); }
		...
		m_pPlayer->RotateToMovingDir(d3dxvDirSum);
	}
