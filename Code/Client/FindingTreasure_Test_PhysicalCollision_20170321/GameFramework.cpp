...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetMesh(CMeshResource::pVoxelMesh);
	...
	m_pPlayer->SetPosition(0.0f, 10.0f, 0.0f);
...
void CGameFramework::ProcessInput()
{
	...
	//D3DXVECTOR3 d3dxvDirSum(0.0f, 0.0f, 0.0f);
	if (GetKeyboardState(pKeyBuffer))
	{
		...
		if (pKeyBuffer[VK_UP] & 0xF0)		m_pPlayer->GoForward(fTimeElapsed);
		if (pKeyBuffer[VK_DOWN] & 0xF0)		m_pPlayer->GoBack(fTimeElapsed);
		if (pKeyBuffer[VK_LEFT] & 0xF0)		m_pPlayer->GoLeft(fTimeElapsed);
		if (pKeyBuffer[VK_RIGHT] & 0xF0)	m_pPlayer->GoRight(fTimeElapsed);
