...
#define DIG_COMPLETE_TIME	2.0f
	...
	if (!bIsDigging)
	{
		fDigTime = 0.0f;
		iLinearIdx = -1;
		if (m_pHavingVoxel && !m_bIsHavingVoxel) {
			m_pHavingVoxel->SetMaterial(CMaterialResource::pStandardMaterial);
			m_pHavingVoxel = NULL;
		}
		return;
	}

	if (!m_pHavingVoxel)			// 가지고 있는 복셀이 없다면, 가질 복셀을 찾는다.
	{
			...
			&& (0 <= d3dxvColLayerRow.z && d3dxvColLayerRow.z < pVoxelTerrain->m_iMaxRow))
		{
					...
					+ (d3dxvColLayerRow.x))]) != NULL)
			{
				...
				if (!pVoxelTerrain->m_ppVoxelObjectsForCollision[
					(int)((d3dxvColLayerRow.y + 1) * pVoxelTerrain->m_iLayerStride
						+ (d3dxvColLayerRow.z) * pVoxelTerrain->m_iRowStride
						+ (d3dxvColLayerRow.x))])
				{
					// 해당 복셀을 소유한다.
					m_pHavingVoxel = pCollidedVoxel;
	...
	{
		D3DXVECTOR3 d3dxvSearchPoint;
		AABB HavingVoxelAABB = CPhysicalCollision::MoveAABB(m_pHavingVoxel->m_pMesh->m_AABB, m_pHavingVoxel->GetPosition());
			...
			&& HavingVoxelAABB.m_d3dxvMin.z < d3dxvSearchPoint.z && d3dxvSearchPoint.z < HavingVoxelAABB.m_d3dxvMax.z)
		{
			...
			if (fDigTime >= DIG_COMPLETE_TIME)
			{
				...
				m_pHavingVoxel->SetMaterial(CMaterialResource::pStandardMaterial);
				m_pHavingVoxel->SetActive(false);
				fDigTime = 0.0f;
				iLinearIdx = -1;
				m_bIsHavingVoxel = true;
			}
			else if (fDigTime >= DIG_COMPLETE_TIME * 0.66f)
			{
				m_pHavingVoxel->SetMaterial(CMaterialResource::pHalfMoreTransMaterial);
			}
			else if (fDigTime >= DIG_COMPLETE_TIME * 0.33f)
			{
				m_pHavingVoxel->SetMaterial(CMaterialResource::pHalfLessTransMaterial);
			}
		}
	}
}
...
{
#define INSTALL_COMPLETE_TIME	1.5f
	...
	if (!m_pHavingVoxel->m_bIsActive)			// 복셀이 설치되지 않았다면. 설치 위치를 찾는다.
	{
		D3DXVECTOR3 d3dxvSearchPoint;
				...
				&& (0 <= d3dxvInspectionIdx.z && d3dxvInspectionIdx.z < pVoxelTerrain->m_iMaxRow))
			{
						...
						+ (d3dxvInspectionIdx.x))])
				{
							...
							+ (d3dxvInspectionIdx.x))])
					{
						...
						m_pHavingVoxel->SetPosition(D3DXVECTOR3(d3dxvColLayerRow.x, d3dxvColLayerRow.y, d3dxvColLayerRow.z));
						m_pHavingVoxel->SetActive(true);
						m_pHavingVoxel->SetMaterial(CMaterialResource::pHalfMoreTransMaterial);
						printf("Installing \n");
	...
	else		// 복셀 설치가 활성화되었다면.
	{
			...
			&& HavingVoxelAABB.m_d3dxvMin.z < d3dxvSearchPoint.z && d3dxvSearchPoint.z < HavingVoxelAABB.m_d3dxvMax.z)
		{
			...
			if (fInstallTime >= INSTALL_COMPLETE_TIME)
			{
				...
				m_pHavingVoxel->SetMaterial(CMaterialResource::pStandardMaterial);
				m_pHavingVoxel->SetActive(true);
				pVoxelTerrain->m_ppVoxelObjectsForCollision[iLinearIdx] = m_pHavingVoxel;
				m_pHavingVoxel = NULL;
				fInstallTime = 0.0f;
				iLinearIdx = -1;
				i = 0;
...
void CPlayer::PushPlayers(CPlayer **m_ppPlayers, int nConnectedPlayersNum)
{
	...
	for (int i = 0; i < nConnectedPlayersNum; i++)
	{
		...
		else if (m_ppPlayers[i])
		{
			...
			m_ppPlayers[i]->m_d3dxvVelocity.x = d3dxvPushVelocity.x * 5.0f;
			m_ppPlayers[i]->m_d3dxvVelocity.z = d3dxvPushVelocity.z * 5.0f;;
		}
