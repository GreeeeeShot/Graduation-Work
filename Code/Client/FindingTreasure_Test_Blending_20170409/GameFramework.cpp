...
void CGameFramework::ProcessInput()
{
	...
	{
		if (pKeyBuffer['S'] & 0xF0 && !m_ppPlayers[0]->m_bIsHavingVoxel) {
		...
		else if (pKeyBuffer['D'] & 0xF0 && m_ppPlayers[0]->m_bIsHavingVoxel)
		{
			m_ppPlayers[0]->m_CameraOperator.ZoomInAtOnce(ZOOM_AT_ONCE_DISTANCE);
		...
		if (pKeyBuffer['A'] & 0xF0)			m_ppPlayers[0]->PushPlayers(m_ppPlayers, 2);
