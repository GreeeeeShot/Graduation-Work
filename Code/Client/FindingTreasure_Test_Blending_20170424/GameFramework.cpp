...
void CGameFramework::BuildObjects()
{
	...
	m_ppPlayers = new CPlayer*[1];

	m_ppPlayers[0] = new CPlayer();
	/*m_ppPlayers[1] = new CPlayer();*/

	m_ppPlayers[0]->SetMaterial(CMaterialResource::pStandardMaterial);
	m_ppPlayers[0]->SetMesh(CMeshResource::pPirateMesh);
	m_ppPlayers[0]->SetShader(CShaderResource::pTexturedLightingShader);
	m_ppPlayers[0]->SetTexture(CTextureResource::pGrassTexture);
	m_ppPlayers[0]->SetBelongType(BELONG_TYPE_BLUE);
...
void CGameFramework::ProcessInput()
{
	...
	if (m_ppPlayers[0]->m_bIsActive)
	{
		...
		{
			if (pKeyBuffer['S'] & 0xF0 && !m_ppPlayers[0]->m_VoxelPocket[m_ppPlayers[0]->m_iVoxelPocketSlotIdx].m_bIsActive) {
				m_ppPlayers[0]->m_CameraOperator.ZoomInAtOnce(ZOOM_AT_ONCE_DISTANCE);
				m_ppPlayers[0]->DigInVoxelTerrain(m_pScene->m_pVoxelTerrain, true, fTimeElapsed);
				m_ppPlayers[0]->m_bIsDigOrInstall = true;
			}
			else if (pKeyBuffer['D'] & 0xF0 && m_ppPlayers[0]->m_VoxelPocket[m_ppPlayers[0]->m_iVoxelPocketSlotIdx].m_bIsActive)
			{
				m_ppPlayers[0]->m_CameraOperator.ZoomInAtOnce(ZOOM_AT_ONCE_DISTANCE);
				m_ppPlayers[0]->InstallVoxel(m_pScene->m_pVoxelTerrain, true, fTimeElapsed);
				m_ppPlayers[0]->m_bIsDigOrInstall = true;
			}
			...
			else
			{
				...
				m_ppPlayers[0]->DigInVoxelTerrain(m_pScene->m_pVoxelTerrain, false, fTimeElapsed);
				m_ppPlayers[0]->InstallVoxel(m_pScene->m_pVoxelTerrain, false, fTimeElapsed);
				m_ppPlayers[0]->m_bIsDigOrInstall = false;
			}
...
void CGameFramework::FrameAdvance()
{
	...
	if (m_ppPlayers[0] && m_ppPlayers[0]->m_bIsActive)
	{
		m_ppPlayers[0]->UpdateShaderVariables(m_pd3dDeviceContext);
		m_ppPlayers[0]->Render(m_pd3dDeviceContext);
	}
	...
	if (m_pUI)
	{
		...
		m_pUI->PlayerHavingVoxelRender(m_pd3dDeviceContext, m_ppPlayers[0]);
	}
