...
void CPhysicalCollision::ImpurseBasedCollisionResolution(CPlayer *pPlayerA, CPlayer *pPlayerB)
{
	...
	// 노멀 속도를 구한다.
	if (fabs(pPlayerA->m_d3dxvVelocity.x) <= 0.1f && fabs(pPlayerA->m_d3dxvVelocity.z) <= 0.1f && (-0.2 >pPlayerA->m_d3dxvVelocity.y))
	{
		pPlayerA->m_d3dxvVelocity.x *= 5.0f;// *(pPlayerA->m_d3dxvVelocity.x - pPlayerB->m_d3dxvVelocity.x);
		pPlayerA->m_d3dxvVelocity.z *= 5.0f;// *(pPlayerA->m_d3dxvVelocity.z - pPlayerB->m_d3dxvVelocity.z);
	}

	if (fabs(pPlayerB->m_d3dxvVelocity.x) <= 0.1f && fabs(pPlayerB->m_d3dxvVelocity.z) <= 0.1f && (-0.2 >pPlayerA->m_d3dxvVelocity.y))
	{
		pPlayerB->m_d3dxvVelocity.x = 5.0f;// *(pPlayerA->m_d3dxvVelocity.x - pPlayerB->m_d3dxvVelocity.x);
		pPlayerB->m_d3dxvVelocity.z = 5.0f;// *(pPlayerA->m_d3dxvVelocity.z - pPlayerB->m_d3dxvVelocity.z);
	}
