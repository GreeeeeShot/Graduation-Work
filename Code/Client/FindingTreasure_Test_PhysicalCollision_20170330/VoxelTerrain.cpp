...
float g_fFriction[2] = { DAMPING_WOOD_0, DAMPING_GRASS_0 };
float g_fReflection[2] = { REFLECTION_WOOD_0, REFLECTION_GRASS_0 };
		...
		for (int row = 0; row < pBlueprint->m_nMaxRow ; row++)
		{
			for (int col = 0; col < pBlueprint->m_nMaxCol ; col++)
			{
				if (iSelectedTexture = pBlueprint->m_iBlueprint[layer][row][col])
				{
					...
					if (iSelectedTexture - 1 < 2) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetTexture(CTextureResource::ppTextureMaterial[iSelectedTexture-1]);
					else m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetTexture(CTextureResource::ppTextureMaterial[1]);
					if(iSelectedTexture -1 < 2) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetMesh(CMeshResource::pVoxelMesh);
					else
					{
						if(iSelectedTexture - 1 == TEXTURE_UPHILL_0) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetMesh(CMeshResource::pUphill000Mesh);
						else if (iSelectedTexture - 1 == TEXTURE_UPHILL_1) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetMesh(CMeshResource::pUphill090Mesh);
						else if (iSelectedTexture - 1 == TEXTURE_UPHILL_2) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetMesh(CMeshResource::pUphill180Mesh);
						else if (iSelectedTexture - 1 == TEXTURE_UPHILL_3) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetMesh(CMeshResource::pUphill270Mesh);
					...
					if (iSelectedTexture - 1 < 2)  m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetFriction(g_fFriction[iSelectedTexture - 1]);
					else m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetFriction(g_fFriction[1]);
					if (iSelectedTexture - 1 < 2) m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetReflection(g_fReflection[iSelectedTexture - 1]);
					else m_ppVoxelObjectsForRendering[m_nVoxelObjectsNum]->SetReflection(g_fReflection[1]);
