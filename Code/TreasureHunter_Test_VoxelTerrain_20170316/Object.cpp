...
void CGameObject::Render(ID3D11DeviceContext *pd3dDeviceContext)
{
	if (m_pShader)
	{
		...
		if(this->m_pMesh->m_pMaterial) m_pShader->UpdateShaderVariable(pd3dDeviceContext, &this->m_pMesh->m_pMaterial->m_Material);
		if(this->m_pMesh->m_pTexture) m_pShader->UpdateShaderVariables(pd3dDeviceContext, this->m_pMesh->m_pTexture);
